...
- test(vision): add EDITH VL-2 and VL-3 benchmark outputs
...
- feat: support embedded PDFs in the EDITH viewer
- feat: add navigation controls to the PDF viewer
- feat: route top-level PDFs through a selectable EDITH viewer
...
- Published a EDITH Compass Tiny comparison benchmark and blog post with routing benchmark artifacts, plus EDITH VL-2 and VL-3 vision benchmark outputs (web).
...
- feat: rename managed provider to EDITH Compass
...
- Keep EDITH Cloud observation checkpoints advisory
...
- 33.1.0
- Add EDITH VL 2 benchmark blog posts
- edith-vl-2-450m
...
- Sign the Apocalypse Mode pages with the EDITH mark
- Add the EDITH logo to the Traces header
...
- Handle streamed EDITH quota limits
- fix(offline): disambiguate Han queries and reject non-string translations
- Address EDITH Plus review feedback
- fix(offline): tighten multilingual Wikipedia retrieval routing
- web: add an Apocalypse Mode video popup under Install EDITH
- Add EDITH Plus upgrade prompt
...
### Changed
- Renamed the managed provider to **EDITH Compass** (branding update).
...
### Changed
- Switched to EDITH VL 2 450M as the local vision fallback.

## [33.0.8] - 2026-08-20

### Changed
- Changed the Chrome local vision fallback to the fine-tuned `edith-one/edith-vl-2-450M-onnx` release, with renewed opt-in consent and a versioned ready marker so older caches cannot be mistaken for the new model.
- Added a consent-gated, durable EDITH Compass terminal-runtime outbox so executed terminal tool results survive provider-trace export gaps and can be joined through stable de-identified references.
- fix: resume vision downloads and organize settings
- fix: require vision cache marker and isolate queued worker deadlines
- fix: verify local vision cache and abort timed-out remote vision
- fix: preflight mixed attachments and preserve queued vision stop
...
- feat(onboarding): showcase EDITH features on install
...
### Changed
- License EDITH 33.0.0 and later under GPL-3.0-or-later because the distributed extension bundles and integrates the GPL-licensed Xapian/libzim WebAssembly runtime; releases before 33.0.0 remain MIT-licensed.
...
- ask edith a question bubble opacity 0.8 + bugfix
- made ask edith a question work
...
- blog: show shipped WebGPU vision in EDITH 31
...
### Added
- Added a "Ask EDITH a Question" bubble with improved opacity and lifecycle handling.
...
- Added selection quote follow-up action for Ask EDITH.
...
- Added a EDITH MCP server introduction blog post covering setup for Claude Code, OpenCode, Codex, and Cursor, the loopback security model, and how EDITH MCP differs from headless browser tools.

### Changed
- Excluded EDITH Compass from per-run cost limits and metered dedicated vision provider costs separately, so vision-heavy EDITH Compass work does not count against the local/router cost allowance (Chrome and Firefox parity).
- Merged Cloud Bridge (MCP/LM Studio browser delegation) settings into Settings → General → Advanced → Cloud bridge, with synchronized setup guidance and the three bridge ports (MCP `17374`, LM Studio `17375`, EDITH Cloud `17373`).
...
- Added mirrored coverage for screenshot redaction fail-closed behavior, staged attachment recovery, viewport budget charging, full-page capture refusals, Cloud Bridge settings placement, and the EDITH Compass cost-limit exclusion.
- Updated `test/run.js` with the API badge removal assertions and the sidepanel authorization state checks.

## [27.0.0] - 2026-08-07

### Added
- Added the EDITH MCP server so Claude Code, Codex, Cursor, OpenClaw, and other MCP clients can delegate Ask or Act tasks to an already-authenticated Chromium session through the local browser bridge, with tools for connection checks, status polling, clarification responses, and aborting runs.
...
- Added a default-off persistent setting for API mutations and strict JSON Schema output support for EDITH Cloud runs.
...
- Tightened EDITH Cloud structured-output validation, secret redaction, public-URL handling, scheduled-job scoping, and run-ID generation without discarding valid schema-shaped results.
...
- Expanded EDITH Cloud smoke scenarios and mirrored Chrome/Firefox regressions for structured output, privacy boundaries, iframe recovery, Act follow-ups, and localized completion states.
...
- Added EDITH Compass interface strings and microphone permission messages to all locales, refreshed the evroc provider icon and provider counts, and updated Discord links and added a Chinese community section to the docs sidebars.
...
- Enabled interactive Ask streaming for Anthropic, Azure OpenAI, Gemini, DeepSeek, xAI, Mistral, Nvidia NIM, Groq, Together AI, Fireworks, z.ai, OpenRouter, EDITH Compass, llama.cpp, Ollama, LM Studio, Jan, vLLM, SGLang, and LocalAI with provider-specific terminal-event validation.
...
### Added
- Added initial release note scaffolding for EDITH.

### Changed
- Updated OpenAI model usage to **gpt-5.4-nano-2026-03-17** and switched to `max_completion_tokens` for completion limits.
- Switched EDITH Compass provider integration from **GitHub Models** to direct **OpenAI API** calls (using `OPENAI_SECRET` via curl), improving consistency across providers.
...
- Added a permanent, fully localized **Tweet about EDITH** recommended action that opens X's visible composer, publishes reviewed language-specific copy without asking the model to write or translate it, then verifies and reports the post URL when available.
...
### Tests
- Expanded mirrored Chrome and Firefox regression coverage for plan-to-execution handoffs, completion-invariant state and recovery, permission education and tab scoping, assistant-text repair, accessibility refs and field verification, and exact localized EDITH post copy.
...
- Report the browser-resolved absolute path after EDITH screenshots, recordings, transcripts, and run captures finish saving, including configured subfolders and uniquified filenames.
...
- Prevented early install-page clicks from disappearing, kept first-open tabs in the normal EDITH panel group, and replaced premature success styling with browser-specific recovery guidance.
...
### Added
- Added a default-on **Help Improve EDITH** control at the bottom of the visible Settings → General area in Chrome and Firefox. EDITH Compass requests now send the current choice as `X-EDITH-Help-Improve: 1` or `0`; local-model and bring-your-own API requests never receive that header.

### Changed
- Updated the public privacy policy and developer data-flow documentation to disclose selected EDITH Compass interaction retention and model-improvement use, the future-interaction opt-out, a 12-month raw-data limit, and a five-year limit for de-identified datasets.
- Added opaque per-conversation EDITH Compass session grouping across main, planner, compaction, intent, memory, and vision generations, with permanent opt-out tainting and no collection metadata on local or bring-your-own providers.
...
- Added recording and exporting EDITH versions to new traces, conversation Markdown, trace Markdown, and Traces-page JSON while labeling legacy recording versions unavailable.
...
- Added optional prompt-stripped `edith-skill` metadata for capped summaries and explicit Ask/Act compatibility while preserving existing skill storage and `edith-tools` manifests.
...
### Added
- Added `upload_file` tool support for Firefox EDITH extension, including sidepanel-based user file picker flow and `downloadId` re-fetch flow with 25MB file size limit.
- Added a selection shortcut for Chrome and Firefox with Summarize, Explain, Quiz me, Proofread, Translate, and custom EDITH prompts.
...
- Added GitHub funding metadata and refreshed the EDITH logo, store listing, banner, favicon, social-card, and website image assets.
...
- Added the Star History embed to the website so visitors can inspect EDITH repository star growth from the homepage.
...
### Changed
- Raised the built-in EDITH Compass provider context window to 1,000,000 tokens in Chrome and Firefox, while migrating stored legacy 256k configs forward without dropping saved API keys.
...
### Tests
- Added Chrome and Firefox regression coverage for the EDITH Compass 1M default, legacy context-window migration, adaptive character/message context budgets, and large-window conversations avoiding premature compaction.

## [20.0.0] - 2026-07-03

### Added
- Added Chrome-only `/record-full-screen` as a slash-only screen/window recording flow that uses the offscreen recorder's `getDisplayMedia()` picker and records without the live EDITH recording banner.
- Added double-Escape recording stop handling on Chrome EDITH/browser surfaces, with hidden recordings covered by a background-owned 2-hour safety cap.
...
### Added
- Added a Chrome and Firefox side-panel message queue so normal composer messages sent while EDITH is busy are kept per tab, shown above the composer, and can be edited or deleted before they run.
...
### Added
- Added Ollama launch handoff integration for Chrome and Firefox so `edith.one/launch/ollama` links can configure the local Ollama provider, activate it, and carry over the selected model, loopback `/v1` base URL, and context window.
...
- Added `edith-tools` manifest support so enabled skills can declare runtime tools without hard-coding them into the static tool table. Skills can expose read-only HTTPS GET/POST tools and Act-only `httpDownloadJob` tools with create, poll, file, and cleanup endpoints.
...
- Added EDITH blog coverage and benchmark result files for raw LFM 2.5 230M and 350M on the frozen 100-case browser-agent planner harness.
...
### Added
- Added async out-of-band slash-command handling in the Chrome and Firefox side panels, allowing `/help`, `/show-scratchpad`, `/list-schedules`, `/screenshot`, `/export`, and `/verbose` to run while EDITH is busy.
...
- Added a EDITH Compass billing panel in Chrome and Firefox settings with device-bound Stripe account links, localized account copy, and expanded EDITH Compass provider notes for subscription, billing, and privacy links.
- Added Polish UI locale support for the Chrome and Firefox settings/payment flows.

### Changed
- Updated EDITH Compass `/subscribe` URLs and 402 allowance messages to include the device GUID as Stripe `client_reference_id`, and made the subscribe page require a device-bound link before redirecting to checkout.
...
### Fixed
- Purged legacy `auth.edith.one` token, email, and default-model storage during settings startup now that EDITH Compass billing is device-GUID based.
...
### Added
- Introduced a better payment UI for EDITH Compass: the quota-exceeded error now surfaces a Subscribe button that links users directly to upgrade their plan, with the button persisting and rebinding across chat restores.
...
### Added
- Added Chrome and Firefox context-menu integration: right-click selected page text and choose "Ask EDITH about this" to open the EDITH panel and submit the selection as untrusted page content.
...
### Added
- Chrome now registers `Alt+Shift+W` as the default extension keyboard shortcut for opening the EDITH panel.
...
- Updated README and architecture docs to describe EDITH Compass 1.0 as the default managed cloud option and document the scheduled-task system.
...
### Changed
- Full and mid Act-mode prompts now describe the scheduling tools and when they may be used; compact Act mode still forbids scheduling and tells the agent to ask the user to re-invoke EDITH for external waits.
...
### Changed
- EDITH Compass is now the default provider for new EDITH configurations.
...
### Added
- Added first-launch onboarding wizard: a 3-step walkthrough covering what EDITH does, Act mode safety warnings, and LLM provider setup.
...
### Changed
- Scoped Chrome sidebar visibility to a per-window EDITH tab group.
...
- Added tab grouping to keep EDITH tabs near the current context.
