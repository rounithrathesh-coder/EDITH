<p align="center">
  <img src="assets/logo-mark.png" alt="EDITH logo" width="92">
</p>
...
<p align="center">
  <a href="https://chromewebstore.google.com/detail/edith/ljhijonmfahplgbbacgcfnaihbjljhhb"><img src="https://img.shields.io/badge/Chrome-Install-4285F4?style=for-the-badge&amp;logo=googlechrome&amp;logoColor=white" alt="Install EDITH from the Chrome Web Store"></a>
  <a href="https://addons.mozilla.org/firefox/addon/edith/"><img src="https://img.shields.io/badge/Firefox-Install-FF7139?style=for-the-badge&amp;logo=firefoxbrowser&amp;logoColor=white" alt="Install EDITH from Firefox Browser Add-ons"></a>
  <a href="https://microsoftedge.microsoft.com/addons/detail/dfbioajafcijomhljabppcelecgdgfeo"><img src="https://img.shields.io/badge/Edge-Install-0A84FF?style=for-the-badge&amp;logo=microsoftedge&amp;logoColor=white" alt="Install EDITH from Microsoft Edge Add-ons"></a>
</p>
...
<p align="center">
  ...
  <a href="docs/">Docs</a> ·
  <a href="https://edith.one">Website</a> ·
  <a href="https://discord.gg/cgC325ssfw">Discord</a> ·
  <a href="LICENSE">GPL-3.0-or-later</a>
</p>

![EDITH reading a page, filling in a form, and fetching a file](assets/edith-demo.gif)

EDITH is a web browser extension that puts an AI agent in a side panel next to
...
Install from the [Chrome Web Store](https://chromewebstore.google.com/detail/edith/ljhijonmfahplgbbacgcfnaihbjljhhb),
[Firefox Add-ons](https://addons.mozilla.org/firefox/addon/edith/), or
...
```bash
git clone https://github.com/edith-one/edith.git
```

**Chrome** — open `chrome://extensions/`, enable **Developer mode** (top
right), click **Load unpacked**, and select the `edith/src/chrome` folder.
...
Click the EDITH icon to open the side panel, then type something like:
...
**EDITH Compass 1.0** is the default and needs no API key or local setup.

**Local models** need no API key either. Point EDITH at any OpenAI-compatible
server:
...
schemas. EDITH auto-detects the real window for llama.cpp, Ollama, and LM
...
preview `ollama launch edith --model <model>` handoff. Details:
...
EDITH separates **tier** from **mode**. Tier (`compact`, `mid`, `full`) is a
...
Chrome side panel shortcuts work when the EDITH side panel has focus.
...
| `Escape` twice                  | Stop an active recording from EDITH or browser pages                      |
...
| [Export and workflow formats](docs/export-and-workflow-formats.md)                                                       | `edith-config/1`, `edith-workflow/1`               |
...
Chat about everything EDITH — help, local and cloud model setups, site
adapters, show-and-tell, and contributor coordination — on the
[EDITH Discord](https://discord.gg/cgC325ssfw). See
...
[GitHub issues](https://github.com/edith-one/edith/issues), not Discord.
...
can delegate a task to EDITH running in the session you are already signed
...
```bash
npx -y @edith/mcp-server
```

Once the server is running, open **EDITH → Settings → General → Advanced →
...
```
edith_run(task: "open the Stripe dashboard and list last week's failed
             payments with amounts and customer emails", mode: "ask")
```

Use `edith_extract` with a JSON Schema when the caller needs predictable
...
rather than the ~50 low-level browser primitives: EDITH's permission gate
...
> The extension holds **one** bridge socket at a time — EDITH Cloud (17373),
...
[`edith/web-tools`](https://lmstudio.ai/edith/web-tools):

```bash
lms clone edith/web-tools
```
...
<a href="https://github.com/edith-one/edith/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=edith-one/edith" />
</a>
...
@software{edith2026,
  author = {Sokullu, Emre},
  title = {EDITH: Open-source AI browser agent for chatting with pages},
  year = {2026},
  publisher = {GitHub},
  url = {https://github.com/edith-one/edith}
}
```

## License

EDITH 33.0.0 and later is licensed under
...
Built with ❤️ by [Emre Sokullu](https://emresokullu.com) and [open-source contributors](https://github.com/edith-one/edith/graphs/contributors).
