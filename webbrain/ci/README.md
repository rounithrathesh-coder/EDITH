...
`ci/` runs catalogued browser-agent scenarios in a fresh EDITH Cloud
...
- `EDITH_BASE_URL` defaults to `https://edith.cloud`.
...
`edith_config_result.enforced`; a managed-but-unattested setting fails the
...
1. EDITH run status and schema-valid `done_json` output.
