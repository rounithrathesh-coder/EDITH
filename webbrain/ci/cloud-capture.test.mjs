...
const chromeApi = {
  storage: {
    session: {
      async get() { return { edithCloudRunSnapshots: storedRows }; },
      async set(value) { storedRows = value.edithCloudRunSnapshots; },
    },
    local: {
      async get() { return {}; },
    },
...
const controller = createCloudRunController({
  ...
  stopRecording: async (options) => {
    ...
    return { ok: true, filename: 'edith-ci-run_capture_fixture.webm' };
  },
});
...
assert.equal(calls[0][2].filename, 'edith-ci-run_capture_fixture.webm');
assert.deepEqual(calls[1], ['stop', { expectedRecordingId: 'rec_fixture' }]);
assert.equal(snapshot.updates.at(-1).type, 'artifact');
assert.equal(snapshot.updates.at(-1).data.filename, 'edith-ci-run_capture_fixture.webm');
