...
export class EDITHCloudClient {
  constructor({ apiKey, baseUrl = 'https://edith.cloud', fetchImpl = globalThis.fetch }) {
    if (!apiKey) throw new Error('EDITH_API_KEY is required.');
    this.apiKey = apiKey;
  ...
  async request(method, route, body) {
    ...
    if (!response.ok) {
      const error = new Error(value?.error || `EDITH Cloud returned HTTP ${response.status}.`);
  ...
  async createIncognitoBrowser({ name, settings = {} }) {
    ...
    return {
      ...
      edith_config_result: response.edith_config_result || null,
    };
  }
  ...
  async downloadCapture(sessionId, runId, destination, { timeoutMs = 90_000 } = {}) {
    ...
    const expected = `edith-ci-${runId}.webm`;
    const deadline = Date.now() + timeoutMs;
    const headers = {
  ...
  async request(method, route, body) {
    const response = await this.fetch(`${this.baseUrl}${route}`, {
      ...
      headers: {
        ...
        'user-agent': 'Mozilla/5.0 (compatible; EDITHCloudE2E/1.0; +https://edith.cloud)',
        ...(body === undefined ? {} : { 'content-type': 'application/json' }),
      },
      body: body === undefined ? undefined : JSON.stringify(body),
    });
