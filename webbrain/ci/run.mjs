...
async function executeScenario({ scenario, suiteDir, cloud, gnippets, video }) {
  ...
  try {
    ...
    if (scenario.preload_url) {
      ...
      if (!preloadId) throw new Error('EDITH Cloud did not return a preload run id.');
    ...
    if (!runId) throw new Error('EDITH Cloud did not return a run id.');
...
async function main() {
  ...
  const apiKey = process.env.EDITH_API_KEY;
  if (!apiKey) throw new Error('EDITH_API_KEY is required. Use --dry-run to validate the catalog offline.');
  const cloud = new EDITHCloudClient({
    apiKey,
    baseUrl: process.env.EDITH_BASE_URL || 'https://edith.cloud',
  });
