...
import { GnippetsE2EClient, EDITHCloudClient } from './lib/edith-client.mjs';
...
const smokeWorkflow = await fs.readFile(
  path.join(root, '..', '.github', 'workflows', 'edith-cloud-smoke.yml'),
  'utf8',
);
...
const cloudClient = new EDITHCloudClient({
  apiKey: 'test-cloud-key',
  baseUrl: 'https://edith.example',
    ...
    updates: [
      {
        ...
        data: {
          ...
          args: {
            ...
            prompt: 'Use EDITH CI A with a random shirt size and random harmless comment. Use set_field and verify_form, submit exactly once, and finish only after the response was recorded.',
          },
        },
      },
      { type: 'tool_result', data: { name: 'schedule_task', result: { success: true, jobId: 'task_1' } } },
      {
        ...
        data: {
          ...
          args: {
            ...
            target: { type: 'url', url: 'https://forms.gle/nDSbn2B6Cym4x9Bi8' },
            prompt: 'Use EDITH CI B with a random shirt size and random harmless comment. Use set_field and verify_form, submit exactly once, and finish only after the response was recorded.',
          },
        },
      },
...
assert.match(diagnosticRequest.options.headers['user-agent'], /EDITHCloudE2E/);

const sensitiveTrace = sanitizeTrace({
  format: 'edith.run-trace',
  version: 1,
  run: {
...
let clarifyPolls = 0;
const staleClarifyClient = new EDITHCloudClient({
  apiKey: 'test-cloud-key',
  baseUrl: 'https://edith.example',
  fetchImpl: async () => ({
    ok: true,
