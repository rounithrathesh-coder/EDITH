# EDITH — Agent Security Threat Model & Mitigations
...
3. Where does inference run, and what leaves the device? (The selected provider determines this: EDITH Compass and bring-your-own cloud providers receive the request context; local model runtimes keep inference on the machine, while a local gateway may forward it upstream.)
