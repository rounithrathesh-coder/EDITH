...
This guide walks through adding a new tool to the EDITH agent — from schema definition to execution dispatch to result handling.
...
- **Core tool**: product-owned browser, DOM, network, download, scheduler, or
  privileged behavior implemented in EDITH source. Use the full checklist
  below.
- **Skill tool**: user-importable, removable HTTP or download-job integration
  declared in a skill's `edith-tools` manifest. Use this when the tool is
  best treated as a trusted third-party extension rather than a EDITH core
  primitive.
...
Add a fenced `edith-tools` JSON block to the skill markdown:
...
`edith-skill` is optional and is stripped from the loaded prompt. Its
...
Without metadata, EDITH infers the first prose paragraph as the summary and
