...
Apocalypse Mode is EDITH's optional offline knowledge layer. It reads
...
On supported Chromium browsers, EDITH VL 2 450M is an optional local vision fallback.
...
Archive language is selected independently from EDITH's interface language.
...
Before an install, EDITH resolves the archive's Metalink and shows its exact byte size,
...
When the current catalog or archive omits a license field, EDITH says that it
...
- IndexedDB (`edith_apocalypse_mode`) contains the opt-in setting, archive
...
- Archive bodies are kept in the extension's Origin Private File System (OPFS),
  ...
  archive from EDITH retains the user-owned file; Firefox uses OPFS.
...
- The Chromium-only local vision model uses the browser's Transformers cache.
  ...
  released until EDITH actually needs local screenshot analysis.
- An installed archive that later becomes unreadable because of corruption,
  ...
  EDITH reports the read failure instead of misreporting an empty search.
...
EDITH reads the documented openZIM structures directly, using the MIT-licensed
