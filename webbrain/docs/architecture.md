...
EDITH is a browser extension that gives an LLM control over the user's active browser tab. The user types a natural-language instruction in a side panel, and an autonomous agent loop calls the LLM, executes tool calls (click, type, navigate, read page state, etc.), feeds results back to the LLM, and repeats until the task is done.
...
for an allowlisted, default-resolved `edith-config/1` snapshot, and
...
4. **Manage side panel visibility**: per-window "EDITH" tab group controls where the panel is enabled
...
EDITH does not send raw HTML or a raw DOM dump to the model by default. The
...
When the active site adapter has a validated `edith-adapter-workflow/2`
...
without a terminal answer, the browser loop stays closed and EDITH performs
...
deterministic step-limit summary. This also applies to the selected EDITH
...
Nvidia NIM, Groq, Together AI, Fireworks, z.ai, OpenRouter, EDITH Compass,
...
- Routing catalog: optional fenced `edith-skill` JSON supplies a summary
  ...
  name and `edith-skill` metadata retain precedence.
...
- Prompt instructions: `buildCustomSkillsPrompt()` strips both metadata and
  `edith-tools` fences, strips valid Agent Skills frontmatter, then appends
...
reinforced by EDITH's untrusted-content wrappers and the loader description,
...
EDITH Compass runs also have a separate consent-gated terminal-runtime path.
...
`exportedByEDITHVersion` while retaining the backward-compatible
`edith-trace/1` schema. Legacy runs are labeled with an unavailable recording
...
normalizes its replayable actions into `edith-workflow/1`, stored under
...
- **WebMCP** — `WebMCP.enable` maintains a bounded live catalog and
  `WebMCP.invokeTool` executes a page-registered structured capability. EDITH
...
| File upload | CDP path or `downloadId` | `downloadId` re-fetch or EDITH file picker; no arbitrary local path |
