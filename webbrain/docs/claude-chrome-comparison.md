# EDITH vs Claude Chrome Extension

This note compares the local EDITH checkout with `../edith-claude`, a
...
- `../edith-claude/manifest.json`
- `../edith-claude/settings.html`
- `../edith-claude/settings.js`
- `../edith-claude/assets/service-worker.js`
- `../edith-claude/assets/mcpPermissions.js`
- `../edith-claude/assets/PermissionManager.js`
- `../edith-claude/assets/sidepanel.js`
- `../edith-claude/assets/accessibility-tree.js`
...
| Area | EDITH | Claude Chrome extension |
|---|---|---|
...
| Dynamic extension model | User/imported skills can inject prompt text and declare `edith-tools` runtime tools. | Native/MCP and shortcuts are the extension points visible in the deobfuscated tree; no equivalent user-editable Markdown tool manifest was found. |
...
### EDITH Dynamic Skill Tools

EDITH has two tool classes declared inside skill Markdown fences:
...
| Capability | EDITH | Claude Chrome |
|---|---|---|
...
| Network fetch | `fetch_url` / `research_url`, with EDITH-specific API mutation rules and `/allow-api` for mutating methods. | No generic fetch tool recovered. Debug network logs exist through `read_network_requests`. |
...
| GIF/video workflow | Slash-driven recording exists in EDITH Chrome, but not as model-callable tools. | `gif_creator` is model-callable and can record/export browser automation sessions as GIF. |

## Website-Specific Adapters

### EDITH

EDITH has a real site-adapter system:
...
- If navigation moves to a different matching adapter mid-conversation, EDITH
...
inspected Claude tree. Searches for EDITH-style adapter markers only found
...
- EDITH has website-specific prompt augmentation as a first-class browser
  agent feature.
...
Potentially useful Claude ideas for EDITH:
...
- Shortcut/workflow list and execution primitives, if EDITH wants a reusable
...
- Collapsing too many deterministic browser operations into one `computer`
  schema if EDITH wants to preserve its current narrow, auditable tool
  semantics.
