...
EDITH can export a conversation, a recorded tool chain, Settings, or a saved
workflow. These files have different privacy and compatibility properties.

| Command or UI | File | Format | Treat as sensitive? |
|---|---|---|---|
| `/export` | `edith-chat-<timestamp>.md` | Conversation Markdown | Yes. It contains visible chat and system messages. |
| `/export --traces` | `edith-traces-<timestamp>.md` | Recorded tool-chain Markdown | Yes. It can contain user prompts, model output, tool arguments, URLs, and results. Raw system prompts are not embedded. |
| `/export --config` | `edith-config-<timestamp>.json` | `edith-config/1` | **Yes. It is plaintext and can contain API keys, profile data, and user memory.** |
| `/workflow --export <id>` | `<name>.edith-workflow.json` | `edith-workflow/1` | Review before sharing. Runtime values are omitted, but saved targets and URL scopes remain. |
| Traces page **Export JSON** | `edith-trace-<model>-<run-id>.json` or `edith-session-<session-id>.json` | `edith-trace/1` | Yes. It contains one recorded run or a session bundle and may include screenshots. |
...
```json
{
  "schema": "edith-trace/1",
  "run": {},
  "events": [],
  "exportedAt": 1784937600000,
  "exportedByEDITHVersion": "25.8.5"
}
```
...
changing the `edith-trace/1` schema:
...
```bash
node scripts/trace-to-atif.mjs edith-trace-model-run.json
```

By default this writes `edith-trace-model-run.atif.json` next to the source
...
its source run in `extra.edith_run_id`.
...
files referenced alongside the trajectory, while a EDITH JSON export embeds
...
`edith-trace/1` run through an OpenTelemetry-compatible observability
pipeline:

```sh
npm run trace:otlp -- edith-trace-example.json \
  --output edith-trace-example.otlp.json
```
...
`ExportTraceServiceRequest`. Legacy input contains one `invoke_agent EDITH`
...
curl -H 'Content-Type: application/json' \
  --data-binary @edith-trace-example.otlp.json \
...
| Field | Meaning |
|---|---|
| `schema` | Required format identifier. Imports accept `edith-config/1`. |
| `exportedAt` | ISO 8601 export time. |
| `edithVersion` | Version of the extension that created the file. |
...
device-bound EDITH Compass or Cloud Sync identity and session data.
...
## Portable workflows: `edith-workflow/1`
...
| Field | Meaning |
|---|---|
| `schema` | Required format identifier. Imports accept `edith-workflow/1`. |
...
- Read `schema` before consuming JSON. A different schema string is a different
  format version.
- Use `edithVersion` or `source.edithVersion` for diagnostics, not as a
