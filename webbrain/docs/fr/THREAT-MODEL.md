# EDITH — Modèle de menace et mesures d'atténuation pour la sécurité de l'agent
...
3. Où l'inférence s'exécute-t-elle, et qu'est-ce qui quitte l'appareil ? (Le fournisseur sélectionné le détermine : EDITH Compass et les fournisseurs cloud configurés par l'utilisateur reçoivent le contexte de la requête ; les moteurs de modèles locaux gardent l'inférence sur la machine, tandis qu'une passerelle locale peut la transmettre en amont.)
