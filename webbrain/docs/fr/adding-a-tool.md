...
Ce guide explique comment ajouter un nouvel outil à l'agent EDITH — de la définition du schéma à la répartition de l'exécution en passant par la gestion des résultats.
...
- **Outil central** : navigateur, DOM, réseau, téléchargement, planificateur ou comportement privilégié appartenant au produit et implémenté dans le code source de EDITH. Utilisez la liste de contrôle complète ci-dessous.
- **Outil de compétence** : intégration HTTP ou de tâche de téléchargement importable et supprimable par l'utilisateur, déclarée dans le manifeste `edith-tools` d'une compétence. Utilisez ceci lorsque l'outil est mieux traité comme une extension tierce de confiance plutôt qu'une primitive centrale de EDITH.
...
Ajoutez un bloc JSON `edith-tools` délimité dans le markdown de la compétence :
