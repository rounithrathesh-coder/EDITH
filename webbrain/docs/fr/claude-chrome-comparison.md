# EDITH vs Extension Claude Chrome

Cette note compare le checkout EDITH local avec `../edith-claude`, un
...
| Domaine | EDITH | Extension Claude Chrome |
|---|---|---|
...
### Outils de compétences dynamiques EDITH

EDITH dispose de deux classes d'outils déclarées dans les blocs Markdown de
compétences :
...
| Requête réseau | `fetch_url` / `research_url`, avec des règles de mutation d'API spécifiques à EDITH et `/allow-api` pour les méthodes mutantes. | Aucun outil de requête générique trouvé. Des logs réseau de débogage existent via `read_network_requests`. |
...
| Flux de travail GIF/vidéo | L'enregistrement via slash existe dans EDITH Chrome, mais pas en tant qu'outils appelables par le modèle. | `gif_creator` est appelable par le modèle et peut enregistrer/exporter des sessions d'automatisation navigateur en GIF. |

## Adaptateurs spécifiques aux sites

### EDITH

EDITH dispose d'un véritable système d'adaptateurs de sites :
...
- Si la navigation passe à un adaptateur correspondant différent en cours de
  conversation, EDITH injecte un nouveau message utilisateur
...
Les recherches de marqueurs d'adaptateurs de style EDITH n'ont trouvé que
...
- EDITH dispose d'une augmentation d'invite spécifique au site en tant que
...
Idées potentiellement utiles de Claude pour EDITH :
...
- Des primitives de liste et d'exécution de raccourcis/flux de travail, si
  EDITH souhaite une couche de flux de travail réutilisable distincte des
...
- Regrouper trop d'opérations navigateur déterministes dans un seul schéma
  `computer` si EDITH souhaite préserver sa sémantique d'outils étroite,
