...
EDITH permet à l'extension de répondre à des questions en utilisant des
...
- **Corpus de textes Emergency Box.** Un ZIP vérifié d'environ 502 Mo contenant
  ...
  texte source) distribué depuis le dépôt `edith-one/emergency-box-corpus`.
...
4. **Fusionner et budgéter.** Les résultats sont fusionnés, diversifiés et
   ...
   ce budget dans son raisonnement, EDITH réessaie avec un prompt de
...
- **OPFS** (Origin Private File System) :
  - `.edith-offline-rag-sahpool-v1/` — répertoire du pool SAH SQLite
  - `edith-offline-rag/emergency-box-text/downloads/` et `installs/` — fichiers du corpus d'urgence
- **IndexedDB** (`edith_offline_rag`) : état du cycle de vie du corpus, version active, manifeste, ID d'installation, chemin d'index, déclaration d'index vectoriel
- **IndexedDB** (`edith_emergency_box`) : enregistrements PDF/ressources installés utilisés pour les liens **Open PDF**
...
licences permissives et n'imposent pas eux-mêmes de copyleft. EDITH 33.0.0
