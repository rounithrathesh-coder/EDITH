...
- **EDITH Compass** : les requêtes passent par `api.edith.one` ; Aider à
  améliorer EDITH est activé par défaut et, tant qu'il reste activé,
...
- **Fournisseurs cloud configurés par l'utilisateur** : OpenAI, Anthropic,
  ...
  et ne sont jamais collectées par EDITH
- **Moteurs de modèles locaux** : llama.cpp, Ollama, LM Studio, Jan, vLLM,
  ...
  de l'utilisateur
- **Proxy local compatible OpenAI** : EDITH contacte seulement la passerelle
...
sont jamais collectées par EDITH. Les requêtes EDITH Compass sont traitées
et peuvent être conservées conformément à la section détaillée de la
[documentation anglaise](../privacy-and-data-flow.md#edith-compass-improvement-data).
...
synchronisées séparément avec EDITH.
...
l'un de ces outils, EDITH envoie uniquement l'URL actuelle ou fournie par le
