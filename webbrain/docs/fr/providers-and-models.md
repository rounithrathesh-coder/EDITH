...
EDITH ajoute 76 cartes désactivées par défaut depuis l’instantané du
...
directement ; si elle est absente, EDITH enregistre une estimation prudente
...
- **Ollama** : `http://localhost:11434/v1` — `ollama serve`, ou `ollama launch edith --model <model>`
...
`./cli-proxy-api --config ./config.yaml`. Dans EDITH, conservez l'URL
...
EDITH lit les métadonnées natives du modèle sélectionné avant l'enrichissement et
...
<p align="center">
  <img src="../../web/assets/edith-ollama-heart.png" alt="EDITH adore le relais de lancement Ollama" width="720">
</p>

EDITH prend aujourd'hui en charge Ollama via le fournisseur local compatible
OpenAI. Un nouveau relais `ollama launch edith --model <model>` peut aussi
configurer EDITH automatiquement, mais il n'est pas encore intégré à Ollama
en amont. Pour l'instant, essayez-le depuis la [branche
`codex/ollama-edith-launch-handoff` de
...
Les entrées de fournisseur obsolètes (`edith`, `openai_subscription`,
`claude_subscription`) sont filtrées.

### Plafonds de Coût

Les paramètres exposent des plafonds de coût cloud pour la session et le total. L'agent préfère une valeur `usage.cost`/`usage.cost_usd` rapportée par le fournisseur lorsqu'elle est présente (OpenRouter la rapporte directement). Pour les fournisseurs cloud directs qui ne retournent que des compteurs de tokens, EDITH estime les dépenses à partir des champs de configuration du fournisseur :
