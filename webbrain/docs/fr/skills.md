...
accompagné de son propre manifeste d'outils — que EDITH charge dans une
...
Un bloc JSON `edith-skill` optionnel peut déclarer :
...
EDITH reconnaît également les métadonnées YAML obligatoires `name` et
...
un bloc `edith-skill` restent prioritaires.

Cette compatibilité concerne uniquement les instructions. EDITH importe un
...
d'outils EDITH. Utilisez `edith-tools` pour les outils HTTP EDITH.
EDITH ne reconnaît les blocs `edith-skill` et `edith-tools` que dans
...
tâche de téléchargement de courte durée, via un manifeste JSON `edith-tools`.
...
Réécrit la prose que EDITH rédige pour vous, par exemple une réponse à un
