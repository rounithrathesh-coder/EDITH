...
EDITH accepte les commandes slash en tant que premier élément d'une ligne dans
...
| `/workflow --export <id>` | Télécharger un fichier JSON portable `edith-workflow/1` assaini |
...
| `/dangerously-skip-permissions` | **Contournement global des demandes d'autorisation.** Désactive `Ask before consequential actions` sans ouvrir les Paramètres. EDITH agira sans demandes par site jusqu'à ce que vous réactiviez le réglage. |
...
renvoie plusieurs fois une image vide en arrière-plan, EDITH l'écarte et
...
`checkout-before.png` et `checkout-after.png` ; sans `--save-as`, EDITH
...
- **Les instantanés de Paramètres** utilisent `edith-config/1` et incluent
...
- **Les workflows enregistrés** utilisent un schéma distinct
  `edith-workflow/1` ; ce ne sont pas des rejeux bruts de traces. Les valeurs
  ...
  liée à l'origine et à la famille d'URL enregistrées. À l'exécution, EDITH
...
- **Les fichiers de workflow portables** contiennent la définition
  `edith-workflow/1` brute assainie et sont limités à 1 Mio. L'export
  ...
  sécurité entre Chrome, Firefox et EDITH Cloud.
