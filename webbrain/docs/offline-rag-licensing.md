...
For a reproducible source build, EDITH pins the versions declared by the
...
continue through EDITH's existing audited ZIM reader.
...
2. a prominent notice that the package contains a modified javascript-libzim /
   libzim / Xapian Wasm build and the exact EDITH modifications and date;
...
6. a documented determination about the license of the combined extension and
   any required source release or relicensing of EDITH itself.
...
- EDITH **33.0.0 and later**, including the repository and Chrome, Edge, and
...
- `make libzim_release` downloads a **prebuilt** `libzim_wasm-emscripten-9.8.1`
  ...
  The upstream release workflow uses this. EDITH must not, because the
...
EDITH builds the source path. CI must archive every downloaded tarball and
...
`dist/edith-*-corresponding-source.zip` asset from that exact `HEAD`; the
...
EDITH's own reader answers the question instead: `hasFullTextIndex()` on the
