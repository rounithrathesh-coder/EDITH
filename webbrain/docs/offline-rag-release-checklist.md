...
`edith-one/emergency-box-corpus` GitHub repository. The browser imports the
...
3. In browser developer tools, set the network to Offline (or disconnect the
   machine), close all EDITH pages, and restart the browser.
...
- [ ] If GPL was approved, complete corresponding source, build scripts,
      ...
      includes `edith-*-corresponding-source.zip` beside the browser ZIPs.
