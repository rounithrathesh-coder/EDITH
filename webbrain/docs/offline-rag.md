...
EDITH's offline retrieval-augmented generation (RAG) pipeline lets the
...
- **Emergency Box text corpus.** A verified ~502 MB ZIP containing ~570
  ...
  the `edith-one/emergency-box-corpus` repository. Documents are PDF-derived
...
4. **Fuse and budget.** Hits are fused, diversified, and wrapped as untrusted
   ...
   budget inside reasoning, EDITH retries with a shorter evidence prompt
...
- **OPFS** (Origin Private File System):
  - `.edith-offline-rag-sahpool-v1/` — SQLite SAH pool directory
  - `edith-offline-rag/emergency-box-text/downloads/` and `installs/` — Emergency corpus files
- **IndexedDB** (`edith_offline_rag`): Corpus lifecycle state, active version, manifest, install ID, index path, vector index declaration
- **IndexedDB** (`edith_emergency_box`): Installed PDF/resource records used for **Open PDF** citation links
...
permissively licensed and do not independently impose copyleft terms. EDITH
