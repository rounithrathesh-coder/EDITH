...
- **EDITH Compass**: requests go through `api.edith.one`; selected interactions may be retained and used for evaluation, improvement, fine-tuning, and training while Help Improve EDITH is enabled
- **Bring-your-own cloud providers**: OpenAI, Anthropic, Google Gemini, Mistral, DeepSeek, xAI, Groq, OpenRouter, etc. — requests go directly to the provider using the user's credentials and are never collected by EDITH
...
- **Local OpenAI-compatible Proxy**: EDITH connects only to the configured
...
Local-model and bring-your-own API requests are never collected by EDITH. EDITH Compass requests are processed and may be retained as described below.
...
available to the model. When enabled, EDITH may propose sending one unusually
...
After approval, EDITH opens `https://chatgpt.com/` in a visible helper tab
and types only the displayed prompt. EDITH does not separately transmit
...
account and OpenAI settings and policies. EDITH reads the resulting answer
...
answer before the configured wait limit, the delegation fails and EDITH can
...
### EDITH Compass improvement data

Help Improve EDITH is available under Settings -> General and is
on by default. When it is on, EDITH may retain eligible Compass prompts, model
...
and image URLs are excluded from EDITH's improvement database. The extension
sends the current preference, stable conversation id, and an allowlisted
generation label with every EDITH Compass model request. It never attaches
those collection fields to local or bring-your-own providers.

For EDITH Compass only, an enabled Help Improve preference also allows the
...
Current clients explicitly send `X-EDITH-Help-Improve: 1` or `0`. Older
EDITH Compass clients that send neither the preference header nor a session id
...
user's own credentials never pass through EDITH Compass and are never eligible
for EDITH training.

For eligible completed generations, MySQL is EDITH's canonical store. The
...
Before retained Compass interactions are used for model development, EDITH
...
the stored copies are not separately synced to EDITH.

### Trace Recorder

When enabled (Settings → Display → "Record traces"), every agent run is written
to the local `edith_traces` IndexedDB database in one of two privacy tiers:
...
`edith-workflow/1` record in browser local storage
...
`edith-workflow/1` format described above.
...
If a saved locator stops matching, EDITH may show sanitized semantic target
...
workflow preferences. EDITH rejects obvious secrets and credential-like text,
...
`/export --config` creates a local plaintext `edith-config/1` JSON file, and
...
state, the EDITH Compass device ID, conversations, traces, scheduled jobs,
usage counters, or accumulated spend. Import does not upload the JSON to
EDITH Compass or to the configured LLM provider.

### Optional Encrypted Cloud Sync

Active EDITH Compass subscribers may explicitly enable encrypted profile sync in
...
EDITH Compass receives only ciphertext and cryptographic/version metadata. It
...
Settings → General → Advanced before EDITH sends its tool schemas or prompt
guidance to the configured LLM. On supporting Chrome pages, EDITH can then
...
registration frame. EDITH keeps a bounded in-memory per-tab catalog, assigns
...
separate product-telemetry endpoint. When EDITH Compass is selected, the model
request itself goes to `api.edith.one` and is subject to the Compass data-use
...
1. **EDITH Compass model calls** to `https://api.edith.one/v1` (when EDITH Compass is selected; the Help Improve EDITH preference is sent with each request)
...
7. **Encrypted Cloud Sync calls** to `https://api.edith.one/v1/sync` (only after a subscriber explicitly enables sync; vault content is encrypted before upload)
...
Trace records store the EDITH version that created each run. Conversation
...
`exportedByEDITHVersion` plus the run's `edithVersion` when available.
...
EDITH sends only the current or model-provided URL, plus declared options
...
and declares no external endpoint. It guides EDITH's existing page-reading
...
another device, and it forbids sign-in bypasses. When the user asks EDITH to
...
result and model response are also retained locally in the `edith_traces`
...
Model proposes one exact read-only research prompt
  │
  ▼
EDITH consent card shows the exact prompt
