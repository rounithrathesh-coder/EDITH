...
EDITH also ships 78 disabled-by-default provider cards. Most are sourced
...
When a streaming provider returns token usage, EDITH records it directly.
If the provider omits usage, EDITH records a conservative character-based
...
- [`prism-ml/Bonsai-27B-gguf`](https://huggingface.co/prism-ml/Bonsai-27B-gguf)
  ...
  worker. Bonsai is opt-in: EDITH never auto-downloads the 27B weights.
...
`q4f16` ONNX variant, and use a chat template that accepts `tools`; EDITH
...
reasoning template; EDITH keeps text before `</think>` out of the visible
...
- **Ollama**: `http://localhost:11434/v1` — `ollama serve`, or `ollama launch edith --model <model>`
...
In EDITH, open **Settings → Providers → Unsloth Studio (Local)**. Enter the
...
The generic **Local OpenAI-compatible Proxy** card can connect EDITH to a
separately managed [EasyCLIProxyAPI](https://github.com/router-for-me/EasyCLIProxyAPI)
or [CLIProxyAPI](https://github.com/router-for-me/CLIProxyAPI) instance. EDITH
...
canonical [EasyCLIProxyAPI subscription proxy guide](https://edith.one/docs/easy-cli-proxy/).
...
Ollama, llama.cpp, LM Studio, and LocalAI default to `visionMode: auto`. EDITH asks
...
field may be blank, EDITH coalesces concurrent checks but rechecks once per
...
<p align="center">
  <img src="../web/assets/edith-ollama-heart.png" alt="EDITH loves Ollama launch handoff" width="720">
</p>

EDITH supports Ollama today through the local OpenAI-compatible provider. A
new `ollama launch edith --model <model>` handoff can also configure EDITH
automatically, but it is not integrated into upstream Ollama yet. For now, try
it from the [`codex/ollama-edith-launch-handoff` branch of
`esokullu/ollama`](https://github.com/esokullu/ollama/tree/codex/ollama-edith-launch-handoff);
...
./ollama launch edith --model <model>
```
...
Each non-EDITH provider config includes a persisted `configured` flag. An
...
**Selected** for chat. EDITH Compass is always selectable without being marked
...
and forged duplicate entries when loading storage. EDITH Compass and the
...
Deprecated provider entries (`edith`, `openai_subscription`,
`claude_subscription`) are filtered out.

### Cost Allowances

Settings exposes session and total cloud cost allowances. The agent prefers a provider-reported `usage.cost`/`usage.cost_usd` value when present (OpenRouter reports this directly). For direct cloud providers that only return token counts, EDITH estimates spend from the provider config fields:
...
in-browser fallback. It runs `edith-one/edith-vl-2-450M-onnx` through WebGPU in a
