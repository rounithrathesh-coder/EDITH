...
This document describes all remote network downloads performed by EDITH, the remote origins and servers they connect to, the exact triggers and sequence of downloads, integrity verification procedures, and where downloaded assets are stored locally.

---

## 1. Overview & Privacy Principles

EDITH is designed to minimize remote network dependencies. All downloads fall into three categories:
...
| **Local Vision Model** | `huggingface.co` / Hugging Face CDN | `edith-one/edith-vl-2-450M-onnx` weights for local screenshot description and UI analysis | ~810 MB | HTTPS GET (Transformers.js pipeline) | Hugging Face Git LFS SHA-256 hash | Browser Cache API & IndexedDB (`transformers-cache`) |
| **Emergency Text Pack & SQLite Index** | `github.com/edith-one/emergency-box-corpus` (GitHub Releases CDN) | Release assets for curated public-domain field references, prebuilt SQLite FTS5 database, and precomputed E5 embeddings | ~245 MB (compressed ZIP) | Resumable HTTP `Range: bytes={offset}-` streaming fetch | Strict **SHA-256** hash comparison against hardcoded release descriptor before activation | OPFS (`edith-offline-rag/emergency-box-text/`) & IndexedDB (`edith_offline_rag`) |
...
| **Wikipedia ZIM Archive** | `library.kiwix.org` / `download.kiwix.org` / Wikimedia Mirrors | Kiwix openZIM archives containing compressed Wikipedia editions (e.g. Simple English) | ~50 MB – 50+ GB | Metalink XML resolution + piece-by-piece chunked HTTPS download | Chunked **SHA-256** piece verification per Metalink block boundaries | OPFS (`edith_apocalypse_mode`) |
| **Emergency Field PDFs** | `openstax.org`, Internet Archive, or designated mirrors | OpenStax open textbooks and public-domain survival/medical field manuals | 5 – 50 MB per document | Direct streaming HTTPS fetch | Content-length and SHA-256 document hashing | IndexedDB (`edith_emergency_box` store `resources`) |
...
When a user turns on **Apocalypse Mode** (or opens `apocalypse-mode.html` with Apocalypse Mode already enabled), EDITH initiates the offline readiness sequence. The sequence runs as follows:
...
[User Opts In / Enables Apocalypse Mode]
   ...
   └─► 3. Shared Download Tracker (`download-tracker.js`):
          ...
          - Broadcasts state across tabs via `BroadcastChannel('edith-emergency-download-state')`
```
...
1. **Wikipedia Library Selection (`wikipedia-library.html`)**:
   ...
   - EDITH queries `library.kiwix.org` for the OPDS catalog entry and resolves the corresponding `.meta4` (Metalink) file.
...
2. **Emergency Box Field Documents (`emergency-box.html`)**:
   ...
   - EDITH streams the PDF file into the local `edith_emergency_box` IndexedDB store.
...
- When reopened, EDITH issues an HTTP `Range: bytes={bytesReceived}-` header to resume downloading without re-fetching existing bytes.
...
| **IndexedDB (`edith_emergency_box`)** | User-selected Emergency Box PDF field manuals and catalog manifests | Persistent extension storage. |
| **IndexedDB (`edith_apocalypse_mode`, `edith_offline_rag`)** | Archive metadata, download cursor offsets, active version pointers, and filter configurations | Persistent lightweight metadata. |
