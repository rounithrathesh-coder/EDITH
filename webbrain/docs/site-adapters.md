...
`edith-adapter-workflow/2` contract. This is runtime policy, not more page
prose. Before either the compact intent planner or the full planner runs,
EDITH gives it only the active adapter name and a bounded list of app-owned
...
```js
{
  ...
  jobs: ['submit-form'],
  workflow: {
    schema: 'edith-adapter-workflow/2',
    jobs: {
      'submit-form': {
...
| `workflow` | object | Optional validated `edith-adapter-workflow/2` job contract used to tighten runtime completion. |
