...
that EDITH loads into a run **only when it is relevant**. Manage them under
...
An optional fenced `edith-skill` JSON block can declare:
...
EDITH also recognizes the required `name` and `description` YAML frontmatter
...
`edith-skill` block still take precedence.

This is instruction-only compatibility. EDITH imports one text document; it
does not fetch bundled `scripts/`, `references/`, or `assets/`, execute skill
code, or treat the Agent Skills `allowed-tools` field as a EDITH permission
or tool manifest. Use `edith-tools` for EDITH HTTP tools. EDITH
recognizes `edith-skill` and `edith-tools` fences only in the Markdown
...
a fenced `edith-tools` JSON manifest.
...
Rewrites prose EDITH composes for you, such as an email reply or a post, so
