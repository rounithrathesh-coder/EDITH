...
EDITH accepts slash commands as the first thing on a line in the input box.
...
| `/workflow --export <id>` | Download a sanitized portable `edith-workflow/1` JSON file |
...
| `/dangerously-skip-permissions` | **Global permission-prompt bypass.** Turns off `Ask before consequential actions` without opening Settings. EDITH will act without per-site prompts until you re-enable the setting. |
...
EDITH discards it and continues from DOM and accessibility data.
...
`checkout-before.png` and `checkout-after.png`; without `--save-as`, EDITH
uses timestamped filenames.

For the Chrome diagnostic suffix, EDITH may reactivate the originating run
...
- **Settings snapshots** use `edith-config/1` and include all portable
...
- **Saved workflows** use a separate `edith-workflow/1` schema; they are not
  ...
  recorded origin and URL family. At run time EDITH resolves a fresh
...
- **Portable workflow files** contain the raw sanitized `edith-workflow/1`
  ...
  safely move between Chrome, Firefox, and EDITH Cloud.
