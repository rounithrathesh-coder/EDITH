...
EDITH trace data has three independent version layers:
...
- `schema` describes the JSON export envelope. `edith-trace/1` remains the
...
The existing single-run `edith-trace/1` shape remains valid. A session bundle
