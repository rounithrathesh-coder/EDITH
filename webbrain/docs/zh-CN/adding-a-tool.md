...
本指南介绍了向 EDITH 代理添加新工具的完整流程——从模式定义到执行分发再到结果处理。
...
- **核心工具**：产品拥有的浏览器、DOM、网络、下载、调度器或特权行为，在 EDITH 源码中实现。请使用下面的完整清单。
- **技能工具**：用户可导入、可移除的 HTTP 或下载任务集成，在技能的 `edith-tools` 清单中声明。当工具最适合作为受信任的第三方扩展而非 EDITH 核心原语时，使用此方式。
...
在技能 markdown 中添加一个围栏 `edith-tools` JSON 块：
