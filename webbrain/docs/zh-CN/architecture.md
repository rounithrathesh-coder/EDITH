...
EDITH 是一个浏览器扩展，让 LLM 能够控制用户当前活动的浏览器标签页。用户在侧面板中输入自然语言指令，自主代理循环调用 LLM，执行工具调用（点击、输入、导航、读取页面状态等），将结果反馈给 LLM，并重复此过程直到任务完成。
...
4. **管理侧面板可见性**：每个窗口的"EDITH"标签组控制面板启用的位置
...
技能激活后，仅在当前运行中添加：
- 提示指令：`buildCustomSkillsPrompt()` 在将技能文本附加到系统提示之前，剥离围栏的 `edith-tools` 块。
...
每个新轨迹记录都会保存 `edithVersion`。`/export` 包含当前清单版本；`/export --traces` 标注导出版本以及每一轮的记录版本，旧轨迹则标为版本不可用。轨迹页面的 JSON 导出增加 `exportedByEDITHVersion`，同时保留向后兼容的 `edith-trace/1` 模式。
...
Trace 运行时元数据中新增的选区作用域字段只记录作用域策略、锚点是否存在和被排除消息数量，不记录投影文本、锚点指纹或消息指纹。这个白名单不会改变 Trace 其他字段的保留规则；运行、事件、截图、深度调试和无损模式的数据仍按《隐私与数据流》所述的当前隐私模式处理。所有 Trace 和诊断导出都应视为敏感数据。
...
| 文件上传 | CDP 路径或 `downloadId` | 通过 `downloadId` 重新获取或使用 EDITH 文件选择器；不支持任意本地路径 |
