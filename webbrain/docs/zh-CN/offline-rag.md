...
EDITH 的离线检索增强生成（RAG）流水线允许扩展在没有任何网络连接的情况下，使用本地存储的参考材料回答问题。它建立在末日模式（Apocalypse Mode）的 Wikipedia 存档之上，并新增了紧急语料库（Emergency Box）文本集合——一份涵盖医疗、生存、教育和通信领域公共领域参考文档的精选合集。
...
- **紧急语料库文本集。** 一份经过验证的约 502 MB ZIP 文件，包含约 570 份公共领域纯文本文档（约 304 MB 源文本），从 `edith-one/emergency-box-corpus` 仓库分发。文档为多种语言的 PDF 衍生参考资料。已安装的 Emergency Box PDF 是单独的阅读架，**不**走这条 RAG 检索路径。
...
4. **融合与预算。** 命中结果经融合、去冗余后作为不可信证据封装。WebGPU 生成有上限（当前为 2048 个新 token）。如果模型把预算花在推理上，EDITH 会用更短的证据提示重试，而不是编造答案。
...
- **OPFS**（Origin Private File System）：
  - `.edith-offline-rag-sahpool-v1/` — SQLite SAH 池目录
  - `edith-offline-rag/emergency-box-text/downloads/` 和 `installs/` — 紧急语料库文件
- **IndexedDB**（`edith_offline_rag`）：语料库生命周期状态、活跃版本、清单、安装 ID、索引路径、向量索引声明
- **IndexedDB**（`edith_emergency_box`）：已安装 PDF/资源记录，用于 **Open PDF** 引用链接
- **遗留段落向量缓存**：上限 256 MB
...
紧急语料库、SQLite、fflate 和 Transformers.js 均采用宽松许可证，本身不会引入 copyleft 条款。尽管如此，EDITH 33.0.0 及更高版本仍采用 GPL-3.0-or-later，因为发布的扩展集成了采用 GPL 许可证的 Xapian/libzim 运行时。
