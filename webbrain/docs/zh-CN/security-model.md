...
本文档描述了 EDITH 的安全架构 — 扩展能做什么、它信任什么、如何处理凭证，以及如何防御提示注入。
...
| **技能导入边界** | 技能可通过 `edith-tools` 清单暴露只读 HTTP 工具和下载任务工具。导入或保持技能启用是对声明的 HTTPS 端点的信任决策；声明的技能工具使用 `credentials: "omit"` 并应将第三方结果标记为 `resultPolicy: "untrusted"`。下载任务技能工具在保存文件前仍需行动模式和正常的下载权限门。 |
