...
技能是可信的指令文本 —— 可选地带有自己的工具清单 —— EDITH **只在相关时**才会
...
Markdown 正文前会移除 frontmatter。在设置中输入的名称及 `edith-skill` 块仍有
更高优先级。

这仅属于指令兼容。EDITH 只导入一个文本文档；它不会获取打包的 `scripts/`、
`references/` 或 `assets/`，不会执行技能代码，也不会把 Agent Skills 的
`allowed-tools` 字段当作 EDITH 权限或工具清单。EDITH HTTP 工具仍应使用
`edith-tools`。EDITH 只识别有效 frontmatter 之后 Markdown 正文中的
`edith-skill` 和 `edith-tools` 围栏；frontmatter 内形似围栏的文本不能授予
路由资格或注册工具。

## 技能工具

技能可以通过围栏 `edith-tools` JSON 清单暴露只读 HTTP 工具，或短生命周期的
下载任务工具。
...
改写 EDITH 为你撰写的正文，例如邮件回复或帖子，使其读起来像真人所写。它不声明
任何网络工具，也不新增工具。
