...
EDITH 接受作为输入框某行开头的斜杠命令。在面板内输入 `/help` 可查看完整的用法
...
| `/workflow --export <id>` | 下载经过清洗的可移植 `edith-workflow/1` JSON 文件 |
...
| `/dangerously-skip-permissions` | **全局权限提示绕过。** 无需打开设置即可关闭「操作前询问」。在你重新启用该设置之前，EDITH 将不再按站点弹出提示。 |
| `/compact` | 强制压缩当前对话上下文 |
| `/verbose` | 切换详细/压缩工具显示 |
| `/reset` | 清除对话与所有按对话的标志 |
...
标签页。如果 Chrome 多次返回空白的后台画面，EDITH 会丢弃该画面，继续使用 DOM
和无障碍数据。
...
`checkout-after.png`；若不带 `--save-as`，EDITH 使用带时间戳的文件名。
...
- **可移植工作流文件** 包含经过清洗的原始 `edith-workflow/1` 定义，大小上限
  1 MiB。导出会在下载前重新规范化定义。导入会再次规范化，并分配新的本地 ID 和时间戳，
  且永不覆盖已有工作流，因此同一文件可以安全地在 Chrome、Firefox 和 EDITH Cloud
  之间迁移。
