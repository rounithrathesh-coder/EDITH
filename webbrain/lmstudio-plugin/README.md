# EDITH Web Tools — LM Studio plugin

> Listing on LM Studio Hub: `edith/web-tools` (owner `edith`, name `web-tools`,
> public) — https://lmstudio.ai/edith/web-tools
>
...
>
> ```
> lmstudio://plugin?owner=edith&name=web-tools
> ```
>
...
> icon next to the chat input → toggle **edith/web-tools** on). For a CLI
...
**Adds real browser access when the [EDITH extension](https://edith.one)
is installed on a Chromium browser** (Chrome, Edge, Brave, Opera, Vivaldi —
[not Firefox](#why-not-firefox)):

- **`browser_task`** — hand a goal to EDITH running in the browser
...
Same fetching logic the [EDITH](https://edith.one) browser
...
[Hub listing](https://lmstudio.ai/edith/web-tools). For a terminal-based
...
**edith/web-tools** on. Any tool-capable model in the same chat can now call
...
`browser_task` needs the [EDITH extension](https://edith.one) on a
...
2. In **EDITH → Settings → General → Advanced → MCP**, set the URL to
...
> socket. Pointing it here means it is *not* pointed at EDITH Cloud
...
- Connections must send the extension's `hello` frame with
  `client: "edith-extension"`; anything else is closed. This is **not**
...
- `browser_task` delegates a *goal*, never individual clicks. EDITH's
...
- A `timeout` does **not** cancel the run. A task that already submitted a
  ...
  stays visible in the EDITH side panel.
