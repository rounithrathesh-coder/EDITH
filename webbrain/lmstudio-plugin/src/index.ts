 ...
 * EDITH browser extension is attached, real browser delegation.
 *
 ...
 *   - `browser_task` hands a goal to the EDITH extension running in the
...
const browserTaskTool = tool({
  ...
  description:
    ...
    "Requires the EDITH extension (https://edith.one) on a Chromium browser " +
  ...
  parameters: {
    ...
    allowApiMutations: z
      ...
      .optional()
      .describe(
        "Lift EDITH's UI-first rule so it may issue mutating HTTP requests " +
...
const browserStatusTool = tool({
  name: "browser_status",
  description:
    "With no runId, check whether the EDITH extension is connected to this plugin. " +
...
export async function main(ctx: PluginContext): Promise<void> {
  ...
  try {
    ...
    console.error(
      "[edith-lmstudio] browser bridge listener could not start:",
