 ...
 *
 * Scope is task-level on purpose. EDITH's permission gate lives in its agent
...
function resultOf(snapshot: CloudSnapshot, timedOut = false): BrowserTaskResult {
  if (snapshot.status === "needs_user_input") {
    ...
    switch (promptKind) {
      ...
      default:
        return {
          ...
          error: `EDITH returned unsupported prompt kind '${promptKind}'.`,
    ...
    return {
      ...
      clarifyId: String(pending.clarifyId ?? pending.clarify_id ?? ""),
      hint:
        "EDITH paused because a human decision is required. Ask the user this " +
...
async function ensureConnected(
  ...
  return {
    ok: false,
    error: "EDITH browser extension not connected.",
