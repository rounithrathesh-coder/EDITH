/**
 * Bridge client — talks to the EDITH browser extension.
 *
 ...
 *
 * Default port is 17375: EDITH Cloud owns 17373 and the MCP server owns
...
export class BridgeClient {
  ...
  constructor(options: BridgeOptions = {}) {
    this.port = options.port ?? Number(process.env.EDITH_BRIDGE_PORT ?? 17375);
    this.path = options.path ?? (process.env.EDITH_BRIDGE_PATH || "/extension");
    this.commandTimeoutMs =
      options.commandTimeoutMs ?? Number(process.env.EDITH_COMMAND_TIMEOUT_MS ?? 30_000);
  }
  ...
  async ensureStarted(): Promise<void> {
    ...
    this.wss!.on("connection", (socket, request) => {
      ...
      if (this.socket) {
        this.failAllPending(
          new BridgeError(
            "EDITH extension connection was superseded.",
            undefined,
            "COMMAND_INTERRUPTED",
          ),
        );
      ...
      socket.on("close", () => {
        ...
        this.failAllPending(
          new BridgeError(
            "EDITH extension disconnected mid-command.",
            undefined,
            "COMMAND_INTERRUPTED",
          ),
        );
  ...
  notConnectedMessage(): string {
    return (
      "No EDITH browser extension is connected, so browser tools are unavailable. " +
      `This plugin is listening on ${this.url}. To connect: install the EDITH ` +
      "extension (https://edith.one) on a Chromium browser — Chrome, Edge, Brave, " +
      "Opera or Vivaldi — open it, then set EDITH → Settings → General → Advanced → MCP to " +
      `${this.url} and enable it. The extension holds one bridge at a time, so it ` +
      "cannot also be pointed at EDITH Cloud (17373) or the MCP server (17374). " +
  ...
  async request<T = unknown>(
    ...
    return new Promise<T>((resolve, reject) => {
      const timer = setTimeout(() => {
        ...
        reject(
          new BridgeError(
            `EDITH did not answer '${action}' within ${responseTimeoutMs}ms.`,
            undefined,
            "COMMAND_TIMEOUT",
          ),
        );
