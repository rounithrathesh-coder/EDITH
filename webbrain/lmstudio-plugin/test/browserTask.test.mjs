...
before(async () => {
  process.env.EDITH_BRIDGE_PORT = String(await freePort());
  process.env.EDITH_COMMAND_TIMEOUT_MS = "50";
  const context = {
  ...
  socket.send(JSON.stringify({ type: "hello", client: "edith-extension" }));
...
test("a replacement socket receives nothing until it sends a valid hello", async () => {
  ...
  await assert.rejects(
    ...
    /No EDITH browser extension is connected/,
  );
  assert.equal(receivedCommands, 0);
