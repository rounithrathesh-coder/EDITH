/**
 * Bridge server — the local endpoint the EDITH extension connects OUT to.
 *
  ...
  async start(): Promise<void> {
    ...
    this.wss!.on("connection", (socket, request) => {
      ...
      if (this.socket) {
        this.failAllPending(
          new BridgeError(
            "EDITH extension connection was superseded mid-command.",
            undefined,
            "COMMAND_INTERRUPTED",
          ),
        );
  ...
  async request<T = unknown>(
    ...
    if (!socket || !this.isConnected()) {
      throw new BridgeError(
        "No EDITH extension is connected. Open the browser, then set " +
          "EDITH → Settings → General → Advanced → MCP to " +
