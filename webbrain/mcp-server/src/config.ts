 ...
 *
 * The bridge port intentionally defaults to 17374, NOT 17373. EDITH Cloud's
...
export const config = {
  ...
  bridgePort: portFromEnv("EDITH_BRIDGE_PORT", 17374),

  /** Path segment the extension connects to. Must match the URL set in EDITH settings. */
  bridgePath: process.env.EDITH_BRIDGE_PATH || "/extension",
   ...
   */
  commandTimeoutMs: durationFromEnv("EDITH_COMMAND_TIMEOUT_MS", 30_000),

  /** Default ceiling for how long `edith_run` will poll before giving up. */
  defaultRunTimeoutMs: durationFromEnv("EDITH_RUN_TIMEOUT_MS", 300_000),

  /** Interval between `cloud_status` polls while a run is in flight. */
  pollIntervalMs: durationFromEnv("EDITH_POLL_INTERVAL_MS", 1_000),
} as const;
