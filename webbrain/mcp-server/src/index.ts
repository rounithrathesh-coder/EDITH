...
server.registerTool(
  "edith_run",
  {
    ...
    description:
      "Delegate a web task to EDITH running in the user's actual browser — already " +
      ...
      "answer with edith_respond — never guess on their behalf.",
    inputSchema: {
      task: z
        .string()
        .min(1)
      ...
      allow_api_mutations: z
        ...
        .default(false)
        .describe(
          "Lift EDITH's UI-first rule so the agent may issue mutating HTTP requests " +
      ...
      timeout_seconds: z
        ...
        .describe(
          "How long to wait before returning control. The run keeps going in the browser " +
            "past this point; poll edith_status to pick it back up.",
        ),
      wait: z
        .boolean()
        .default(true)
      ...
      if (!wait) {
        return ok(
          `Started in the background.\n${describeSnapshot(started)}\n\n` +
            "Poll edith_status with this run_id for progress.",
        );
      }
...
server.registerTool(
  "edith_extract",
  {
    ...
    description:
      ...
      "caller-supplied JSON Schema. This tool always uses EDITH Ask mode, so it cannot " +
      ...
      "prose summary. Use edith_run instead when the task needs interaction.\n\n" +
      "If the run stops with status 'needs_user_input', relay the question to the user and " +
      "answer with edith_respond — never guess on their behalf.",
    inputSchema: {
      task: z
      ...
      timeout_seconds: z
        ...
        .describe(
          "How long to wait before returning control. The extraction keeps running past " +
            "this point; poll edith_status with its run_id.",
        ),
      wait: z
        .boolean()
        .default(true)
...
server.registerTool(
  "edith_status",
  {
    ...
    description:
      "Fetch the current state of a EDITH run, including its result once finished. " +
  ...
  async ({ run_id }): Promise<TextResult> => {
    try {
      ...
      if (runs) {
        if (!runs.length) return ok("No EDITH runs on record.");
        return ok(
          runs
...
server.registerTool(
  "edith_respond",
  {
    ...
    description:
      ...
      "waiting for it to settle. The answer must come from the user — EDITH pauses " +
      ...
      "decision to the exact stable value shown by EDITH: 'once', 'always', or 'deny'. Never " +
...
server.registerTool(
  "edith_connection",
  {
    title: "Check the EDITH browser connection",
    description:
      "Report whether a EDITH extension is currently attached. Call this first when a " +
  ...
  async (): Promise<TextResult> => {
    ...
    return ok(
      `Not connected. Listening on ${bridgeUrl()}, but no extension has dialled in.\n\n` +
        ...
        "EDITH → Settings → General → Advanced → MCP set the URL to\n" +
        `  ${bridgeUrl()}\n` +
        "and enable it. The extension holds one bridge socket at a time, so this cannot " +
        "run at the same time as EDITH Cloud on port 17373.\n\n" +
...
async function main(): Promise<void> {
  ...
  console.error("[edith-mcp] ready on stdio");
}
...
async function shutdown(): Promise<void> {
  ...
  await bridge.stop().catch((error) => {
    console.error("[edith-mcp] shutdown error:", error);
  });
  process.exit(0);
}
...
main().catch((error) => {
  console.error("[edith-mcp] fatal:", error);
  process.exit(1);
});
