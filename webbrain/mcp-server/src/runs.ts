...
import { BridgeError, TERMINAL_STATUSES, EDITHBridge, type CloudSnapshot } from "./bridge.js";
...
export async function abort(bridge: EDITHBridge, runId: string): Promise<CloudSnapshot> {
 ...
 * caller can resume with `edith_status`. Silently killing a half-finished
...
export function describeSnapshot(snapshot: CloudSnapshot, timedOut = false): string {
  ...
  lines.push(`status: ${snapshot.status}${timedOut ? " (still running — poll edith_status)" : ""}`);
  ...
  if (snapshot.status === "needs_user_input" && snapshot.pendingInput) {
    ...
    lines.push("");
    lines.push("EDITH is waiting on a human decision before it continues.");
      ...
      default:
        ...
        lines.push(
          "This prompt kind is unsupported by this client. Do not send a free-form answer; " +
            "update the client before calling edith_respond.",
        );
    }
    if (supported) {
      lines.push(`question: ${question}`);
      lines.push(
        "Relay this to the user and send their answer with edith_respond. " +
