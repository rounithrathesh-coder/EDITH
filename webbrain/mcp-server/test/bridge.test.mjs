...
process.env.EDITH_BRIDGE_PORT = process.env.EDITH_BRIDGE_PORT || "17399";
process.env.EDITH_COMMAND_TIMEOUT_MS = "2000";
process.env.EDITH_POLL_INTERVAL_MS = "20";

const { EDITHBridge, BridgeError, TERMINAL_STATUSES } = await import("../dist/bridge.js");
...
function fakeExtension(url, handler, { client = "edith-extension" } = {}) {
...
test("no extension attached produces an actionable message", async () => {
  ...
  await assert.rejects(
    ...
    (error) => {
      assert.ok(error instanceof BridgeError);
      assert.match(error.message, /No EDITH extension is connected/);
...
test("a new socket cannot inherit an earlier extension handshake", async () => {
  ...
  await assert.rejects(
    ...
    /No EDITH extension is connected/,
  );
