 ...
 * (`EDITH_RUN_TIMEOUT_MS=300000`) crashed the server at startup with
...
const CLEAN = {
  EDITH_BRIDGE_PORT: undefined,
  EDITH_BRIDGE_PATH: undefined,
  EDITH_COMMAND_TIMEOUT_MS: undefined,
  EDITH_RUN_TIMEOUT_MS: undefined,
  EDITH_POLL_INTERVAL_MS: undefined,
};
...
test("durations above the 16-bit port ceiling are accepted", async () => {
  ...
  const { config } = await loadConfig({ ...CLEAN, EDITH_RUN_TIMEOUT_MS: "300000" });
  assert.equal(config.defaultRunTimeoutMs, 300_000);
});
...
test("every duration setting accepts a large value", async () => {
  const { config } = await loadConfig({
    ...CLEAN,
    EDITH_COMMAND_TIMEOUT_MS: "120000",
    EDITH_RUN_TIMEOUT_MS: "3600000",
    EDITH_POLL_INTERVAL_MS: "250000",
  });
...
test("ports keep their 1-65535 bound", async () => {
  await assert.rejects(
    () => loadConfig({ ...CLEAN, EDITH_BRIDGE_PORT: "70000" }),
    /must be a valid TCP port/,
  );
  await assert.rejects(
    () => loadConfig({ ...CLEAN, EDITH_BRIDGE_PORT: "0" }),
    /must be a valid TCP port/,
  );
});
...
test("non-numeric and partially-numeric values are rejected outright", async () => {
  await assert.rejects(
    () => loadConfig({ ...CLEAN, EDITH_BRIDGE_PORT: "abc" }),
    /must be an integer/,
  );
  // parseInt("8080abc") silently yields 8080; that is a typo, not a config.
  await assert.rejects(
    () => loadConfig({ ...CLEAN, EDITH_BRIDGE_PORT: "8080abc" }),
    /must be an integer/,
  );
  await assert.rejects(
    () => loadConfig({ ...CLEAN, EDITH_RUN_TIMEOUT_MS: "5s" }),
    /must be an integer/,
  );
});
...
test("non-positive durations are rejected", async () => {
  await assert.rejects(
    () => loadConfig({ ...CLEAN, EDITH_POLL_INTERVAL_MS: "0" }),
    /positive duration in milliseconds/,
  );
  await assert.rejects(
    () => loadConfig({ ...CLEAN, EDITH_COMMAND_TIMEOUT_MS: "-1" }),
    /positive duration in milliseconds/,
  );
});
