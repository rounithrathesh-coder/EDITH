 ...
 * downloads a prebuilt libzim tarball. EDITH cannot provide corresponding
...
const IMAGE_TAG = 'edith-emscripten-libzim:3.1.41';
...
the exact EDITH driver used to produce the bundled runtime. The build pins
...
Inside a full EDITH checkout, \`npm run build:zim-xapian\` invokes the same
...
function writeRecords(artifacts, correspondingSource) {
  ...
  for (const directory of vendorDirectories) {
    ...
    writeFileSync(path.join(directory, 'README.edith.md'), vendorReadme(artifacts));
  }
...
upstream release asset here: EDITH cannot provide corresponding source for a
...
**GPL-3.0-or-later**. EDITH 33.0.0 and later uses that license because the
distributed extension integrates this runtime. Complete corresponding source for these
binaries is published as a \`edith-zim-xapian-*-corresponding-source.zip\`
