 ...
 *
 * Output:
 *   dist/edith-chrome-<version>.zip
 *   dist/edith-edge-<version>.zip
 *   dist/edith-firefox-<version>.zip
 *   dist/edith-<source-name>-corresponding-source.zip
 *
...
export function correspondingSourceArchivePath(sourceName) {
  return `dist/edith-${sourceName}-corresponding-source.zip`;
}
...
function runCli() {
  ...
  for (const { packageName, sourceDir } of targets) {
    const out = path.join(distDir, `edith-${packageName}-${version}.zip`);
    ...
    const packageLabel = `dist/edith-${packageName}-${version}.zip`;
    ...
    console.log(`  ✓ dist/edith-${packageName}-${version}.zip`);
  }
