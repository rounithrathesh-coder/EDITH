...
store-icon-128.png + the EDITH brand colors. Produces:
...
# ── Brand palette ─────────────────────────────────────────────────────
# Matches edith marketing site + extension settings UI.
...
def render_small_tile(icon: Image.Image) -> Image.Image:
    ...
    draw.text((text_x, 100), "EDITH", font=title_f, fill=TEXT)
    draw.text(
        (text_x, 152),
...
def render_marquee_tile(icon: Image.Image) -> Image.Image:
    ...
    draw.text((text_x, 160), "EDITH", font=title_f, fill=TEXT)
    draw.text(
        (text_x, 296),
        "Open-Source AI Browser Agent",
        font=sub_f,
