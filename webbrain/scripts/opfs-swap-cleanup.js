/**
 * EDITH — OPFS swap-file cleanup (manual, one-off).
 *
 ...
 *
 * HOW TO RUN
 *   1. chrome://extensions → EDITH → "Inspect views: service worker"
 *   2. Paste this whole file into the Console.
 *   3. await EDITH_OPFS.report()      // read-only inventory
 *   4. await EDITH_OPFS.sweepSwap()   // delete every orphaned *.crswap
 *   5. await EDITH_OPFS.dedupe()      // dry run: show redundant full copies
 *      await EDITH_OPFS.dedupe({ dryRun: false })   // keep one, delete the rest
 *
...
(() => {
  ...
  globalThis.EDITH_OPFS = { report, sweepSwap, dedupe, scan };
  console.log('EDITH_OPFS ready — try: await EDITH_OPFS.report()');
})();
