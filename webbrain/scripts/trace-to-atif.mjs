...
/**
 * Convert a Traces-page `edith-trace/1` JSON export to ATIF v1.7.
 *
...
const SOURCE_SCHEMA = 'edith-trace/1';
...
function usageMetrics(usage) {
  ...
  const extra = {};
  // EDITH records provider-native cost. Some providers report USD, while
  // others do not guarantee a currency, so never mislabel it as ATIF cost_usd.
  const reportedCost = finiteNumber(usage.cost);
  if (reportedCost !== undefined) extra.edith_reported_cost = reportedCost;
...
export function edithTraceToAtif(input) {
  if (!isObject(input)) throw new TypeError('EDITH trace export must be an object.');
  if (input.schema !== SOURCE_SCHEMA) {
    throw new TypeError(`Expected schema "${SOURCE_SCHEMA}".`);
  }
  if (Array.isArray(input.runs)) return edithTraceBundleToAtif(input);
  if (!isObject(input.run)) throw new TypeError('EDITH trace run must be an object.');
  if (!Array.isArray(input.events)) throw new TypeError('EDITH trace events must be an array.');
  if (typeof input.run.runId !== 'string' || !input.run.runId.trim()) {
    throw new TypeError('EDITH trace run.runId must be a non-empty string.');
  }

  const run = input.run;
  const runId = String(run.runId).trim();
  const version = String(
    run.edithVersion || input.exportedByEDITHVersion || 'unknown',
  );
  const steps = [];
  ...
  for (const event of [...input.events].sort(eventOrder)) {
    ...
    if (event.kind === 'llm_response') {
      const toolCalls = Array.isArray(data.toolCalls)
        ? data.toolCalls.map((call, index) => {
          ...
          const fallbackId = `edith-${runId}-${eventSeq ?? 'unknown'}-${index + 1}`;
      ...
      const extra = compactObject({
        edith_seq: eventSeq,
        edith_step: finiteNumber(data.step),
      ...
      pendingAgent = {
        step,
        edithStep: finiteNumber(data.step),
        usedCallIds: new Set(),
      };
    ...
    if (event.kind === 'tool') {
      ...
      const pendingCall = pendingAgent?.step.tool_calls?.find((call) => (
        ...
        && (
          pendingAgent.edithStep === undefined
          || finiteNumber(data.step) === undefined
          || pendingAgent.edithStep === finiteNumber(data.step)
        )
      ));
      ...
      let toolCallId = `edith-${runId}-${eventSeq ?? steps.length + 1}-1`;
      let suffix = 2;
      while (allocatedToolCallIds.has(toolCallId)) {
        toolCallId = `edith-${runId}-${eventSeq ?? steps.length + 1}-${suffix}`;
        suffix += 1;
      }
      allocatedToolCallIds.add(toolCallId);
  ...
  if (finalContent && finalContent !== lastAgentMessage) {
    appendStep({
      ...
      extra: { edith_final_content: true },
    });
  }
  ...
  const rootExtra = compactObject({
    ...
    source_exported_by_edith_version: input.exportedByEDITHVersion
      ? String(input.exportedByEDITHVersion)
      : undefined,
  ...
  for (const sourceStep of trajectory.steps) {
    ...
    if (Array.isArray(sourceStep.tool_calls)) {
      step.tool_calls = sourceStep.tool_calls.map((call) => {
        const originalId = String(call.tool_call_id || 'edith-call');
...
function edithTraceBundleToAtif(input) {
  const sessionId = isObject(input.session) && typeof input.session.sessionId === 'string'
    ? input.session.sessionId.trim()
    : '';
  if (!sessionId) {
    throw new TypeError('EDITH trace bundle session.sessionId must be a non-empty string.');
  }
  if (input.runs.length === 0) {
    throw new TypeError('EDITH trace bundle runs must be a non-empty array.');
  }

  const entries = input.runs.map((entry, index) => {
    if (!isObject(entry)) {
      throw new TypeError(`EDITH trace bundle entry ${index} must be an object.`);
    }
    return entry;
  }).sort((left, right) => {
  ...
  const trajectories = entries.map((entry) => ({
    ...
    trajectory: edithTraceToAtif({
      schema: input.schema,
      exportedAt: input.exportedAt,
      exportedByEDITHVersion: input.exportedByEDITHVersion,
      run: entry.run,
      events: entry.events,
    }),
  }));
  ...
  const agentExtra = compactObject({
    edith_run_count: trajectories.length,
  ...
  const version = String(
    input.exportedByEDITHVersion || trajectories[0].trajectory.agent.version || 'unknown',
  );
...
async function main(argv) {
  ...
  const trajectory = edithTraceToAtif(JSON.parse(raw));
