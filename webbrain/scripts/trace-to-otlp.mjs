...
const EDITH_TRACE_SCHEMA = 'edith-trace/1';
...
// collector can reconstruct the EDITH execution graph without guessing.
...
function contentAttributes(run, includeContent) {
  ...
  return [
    ['edith.user.message', boundedContent(run.userMessage)],
    ['edith.final.response', boundedContent(run.finalContent)],
  ];
}
...
function genericEvent(event, includeContent, runStart) {
  ...
  return {
    ...
    attributes: attributes([
      ['edith.event.sequence', finiteNumber(event?.seq)],
      ['edith.event.kind', String(event?.kind || 'unknown')],
      ['edith.step', finiteNumber(data.step)],
      ...(includeContent ? [['edith.event.data', boundedContent(data)]] : []),
    ]),
  };
}
  ...
  return events.flatMap((event) => {
    ...
    if (event?.kind === 'error') {
      return [{
        ...
        attributes: attributes([
          ...base,
          ['exception.type', data.phase ? `edith.${data.phase}` : 'edith.error'],
    ...
    if (event?.kind === 'streaming' || event?.kind === 'note' || event?.kind === 'screenshot') {
      return [{
        timeUnixNano: unixNano(eventTime(event, runStart)),
        name: `edith.${event.kind}`,
        attributes: attributes([
          ...base,
          ['edith.event.status', data.status],
          ['edith.event.reason', data.reason],
          ...(includeContent && event.kind === 'note'
            ? [['edith.note', boundedContent(data.note)]]
            : []),
        ]),
      }];
    }
...
function inferenceSpan(event, context, includeContent) {
  ...
  return {
    ...
    attributes: attributes([
      ...
      ['edith.step', finiteNumber(data.step)],
      ...(includeContent
        ? [['edith.llm.response.content', boundedContent(data.content)]]
        : []),
    ]),
  };
}
...
function toolSpan(event, context, includeContent) {
  ...
  return {
    ...
    attributes: attributes([
      ...
      ['gen_ai.agent.name', 'EDITH'],
      ['edith.tool.result.status', resultStatus],
      ...(data.resultErrorCode ? [['edith.tool.result.error_code', data.resultErrorCode]] : []),
...
export function normalizeTraceExport(input) {
  if (!input || input.schema !== EDITH_TRACE_SCHEMA) {
    throw new Error(`Expected a ${EDITH_TRACE_SCHEMA} export.`);
  }
  const exported = {
    schema: input.schema,
    exportedAt: finiteNumber(input.exportedAt),
    exportedByEDITHVersion: String(input.exportedByEDITHVersion || ''),
  };
...
function buildLineageRecords(bundle) {
  ...
  for (const record of records) {
    ...
    record.spanId = stableHex(`edith:run:${record.runId || record.index}${suffix}`, 16);
  }
  return records;
}
...
function bundleEvent(event, includeContent, runStart) {
  ...
  const base = [
    ['edith.event.sequence', finiteNumber(event?.seq)],
    ['edith.event.kind', kind],
    ['edith.step', finiteNumber(data.step)],
  ];
  let name = isKnownKind(kind) ? `edith.${kind}` : 'edith.unknown';
  const extra = [];
  if (kind === 'error') {
    name = 'exception';
    extra.push(['exception.type', data.phase ? `edith.${data.phase}` : 'edith.error']);
    ...
    if (includeContent) extra.push(['edith.llm.response.content', boundedContent(data.content)]);
  } else if (kind === 'tool') {
    const resultStatus = normalizedToolResultStatus(data.result, data.resultStatus);
    const failed = failedToolResult(data.result, resultStatus);
    extra.push(
    ...
    extra.push(['edith.event.status', data.status], ['edith.event.reason', data.reason]);
    if (includeContent && kind === 'note') extra.push(['edith.note', boundedContent(data.note)]);
  }
  if (!isKnownKind(kind) && includeContent) extra.push(['edith.event.data', boundedContent(data)]);
  return {
    timeUnixNano: unixNano(eventTime(event, runStart)),
    name,
...
function lineageAttributes(record) {
  return [
    ['edith.trace.format.version', record.run.traceFormatVersion],
    ['edith.parent.run.id', record.parentRunId],
    ['edith.parent.session.id', record.parentSessionId],
    ...(record.lineageState !== 'root' && record.lineageState !== 'attached'
      ? [['edith.lineage.state', record.lineageState]]
      : []),
  ];
}

function sessionTraceId(sessionId) {
  return stableHex(`edith:session:${sessionId}`, 32);
}
...
function bundleRunSpan(record, bundle, includeContent) {
  ...
  const span = {
    ...
    attributes: attributes([
      ...
      ['gen_ai.agent.name', 'EDITH'],
      ['gen_ai.agent.version', record.run.edithVersion || bundle.exportedByEDITHVersion],
      ...
      ['edith.run.id', record.run.runId],
      ['edith.run.status', record.run.status],
  ...
  if (record.crossSessionParent) {
    ...
    span.links = [{
      ...
      spanId: parent?.spanId || stableHex(`edith:run:${record.parentRunId}`, 16),
      attributes: attributes([
        ['edith.parent.run.id', record.parentRunId],
        ['edith.parent.session.id', sessionId],
      ]),
    }];
  }
  return span;
  ...
  return {
    resourceSpans: [...groups.entries()].sort(([a], [b]) => a.localeCompare(b)).map(([sessionId, sessionRecords]) => ({
      resource: {
        attributes: attributes([
          ['service.name', 'edith'],
          ['service.version', bundle.exportedByEDITHVersion],
          ['edith.session.id', sessionId],
        ]),
      },
      scopeSpans: [{
        scope: {
          name: 'edith.trace-export',
          ...(bundle.exportedByEDITHVersion
            ? { version: String(bundle.exportedByEDITHVersion) }
            : {}),
        },
...
export function traceExportToOtlp(input, { includeContent = false } = {}) {
  ...
  const traceId = stableHex(`edith:${run.runId || runStart}`, 32);
  const rootSpanId = stableHex(`edith:${run.runId || runStart}:root`, 16);
  ...
  const root = {
    ...
    attributes: attributes([
      ['gen_ai.operation.name', 'invoke_agent'],
      ['gen_ai.agent.name', 'EDITH'],
      ['gen_ai.agent.version', run.edithVersion || input.exportedByEDITHVersion],
      ...
      ['edith.run.id', run.runId],
      ['edith.run.status', run.status],
  ...
  return {
    resourceSpans: [{
      resource: {
        attributes: attributes([
          ['service.name', 'edith'],
          ['service.version', run.edithVersion || input.exportedByEDITHVersion],
        ]),
      },
      scopeSpans: [{
        scope: {
          name: 'edith.trace-export',
          ...(input.exportedByEDITHVersion
            ? { version: String(input.exportedByEDITHVersion) }
            : {}),
        },
