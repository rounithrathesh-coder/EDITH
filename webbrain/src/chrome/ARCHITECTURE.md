...
- optional tool schemas declared in fenced `edith-tools` JSON blocks.
...
recorder context through `getDisplayMedia()`, shows the EDITH recording banner
by default, and can be stopped by its Stop button or double Escape on EDITH or
...
or whole screen if they want the EDITH panel in the video.
...
background.js (on recorder-stop)
      ├─ chrome.downloads.download(dataURL → requested name or edith-recording-<ts>.webm)
...
CDP events are **trusted** (`event.isTrusted === true`). Many sites reject synthetic `el.click()`; CDP is what lets EDITH work on those.
...
`ProviderManager` seeds EDITH Compass, one Chromium in-browser WebGPU provider,
...
Off by default. Enabled via Settings → Display → "Record traces". When on, every agent run writes to an IndexedDB database (`edith-traces`):
