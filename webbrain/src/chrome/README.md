...
- **Continue from Limit** — At the step limit, EDITH first delivers a context-only partial result or explicit blocker; click Continue to keep going
- **Multi-Provider LLM** — EDITH Compass plus local llama.cpp/Ollama/LM Studio/Jan/vLLM/SGLang/LocalAI and major direct cloud providers
- **Reliable Compass improvement traces** — when Help Improve EDITH is enabled, terminal tool outcomes are durably queued and retried without delaying the visible answer
...
3. Click **Load unpacked** → select the `edith/src/chrome` folder
...
3. Navigate to `edith/src/firefox/` and select `manifest.json`
...
# Or run: ollama launch edith --model <model>
...
For llama.cpp, LM Studio, and LocalAI, Vision defaults to **Auto**. EDITH
...
Click the EDITH icon → the side panel opens. Type a message like:
...
- **Protected browser pages** — Browser-internal and extension-store pages restrict content-script/CDP access; EDITH reports these as protected instead of retrying DOM actions.
...
The repository `web/` folder contains the landing page for [edith.one](https://edith.one), deployable to Vercel:
