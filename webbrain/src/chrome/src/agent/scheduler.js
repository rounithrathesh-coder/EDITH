...
function isActiveRunError(error) {
  return /agent run is already in progress|active EDITH run/i.test(String(error?.message || error || ''));
}
...
const SCHEDULED_ASK_SUBSCRIBE_ERROR_RE = /(Subscribe for more usage|Upgrade to EDITH Plus):\s*(https?:\/\/\S+)/i;
...
export class ScheduledJobManager {
  ...
  start() {
    ...
    this.api?.alarms?.onAlarm?.addListener?.((alarm) => {
      ...
      run.catch((e) => {
        console.warn('[EDITH] scheduled job alarm failed:', e);
      });
      return run;
    });
    this.restoreAlarms().catch((e) => console.warn('[EDITH] restore scheduled alarms failed:', e));
  }
    ...
    if (keepWatching) {
      ...
      if (freshAlert) {
        try {
          ...
          console.warn('[EDITH] watch alert playback failed:', error);
        }
      }
      return;
    }
  ...
  async _runJobAttempt(jobId, execution) {
    ...
    try {
      ...
      if (isActiveRunError(e) || e?.code === 'teacher_mode_active') {
        await this._requeue(job, 'The target tab already has an active EDITH run.');
      } else {
    ...
    const onUpdate = (type, data) => {
      ...
      if (type === 'clarify') {
        ...
        this._updateJobIf(job.id, (prev) => prev.status === 'running', () => ({
          ...
          console.warn('[EDITH] failed to mark scheduled job as waiting for input:', e);
        });
        ...
        if (clarifyId && Number.isFinite(deadlineTs) && deadlineTs > 0) {
          this._updateJobIf(job.id, (prev) => (
            ...
            console.warn('[EDITH] failed to extend scheduled clarify timeout:', e);
          });
        }
        ...
        this._updateJobIf(job.id, (prev) => prev.status === 'needs_user_input', () => ({
          ...
          console.warn('[EDITH] failed to resume scheduled job after clarify timeout:', e);
        });
      }
    ...
    try {
      ...
      if (isActiveRunError(e)) {
        await this._requeue(running, 'The target tab already has an active EDITH run.');
      } else {
