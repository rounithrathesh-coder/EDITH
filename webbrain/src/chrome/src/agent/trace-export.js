...
export function tracesToMarkdown(runsWithEvents, {
  title = 'EDITH Conversation — tool chain',
  notes = [],
  exportedByEDITHVersion = '',
} = {}) {
  const runs = Array.isArray(runsWithEvents) ? runsWithEvents : [];
  let md = `# ${title}\n\n`;
  const exportVersion = oneLine(exportedByEDITHVersion);
  if (exportVersion) md += `_Exported with EDITH v${exportVersion}_\n\n`;
  let turnCount = 0;
  let toolCount = 0;
  ...
  for (const entry of runs) {
    ...
    const recordedVersion = oneLine(run.edithVersion || '');
    const events = Array.isArray(entry.events) ? [...entry.events].sort((a, b) => (a?.seq || 0) - (b?.seq || 0)) : [];
    const meta = [
      recordedVersion ? `recorded with EDITH v${recordedVersion}` : 'recorded EDITH version unavailable',
      run.model,
