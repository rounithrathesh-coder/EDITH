...
export const SAVED_WORKFLOW_SCHEMA = 'edith-workflow/1';
...
export function compileWorkflowFromTrace(run, events, options = {}) {
  ...
  const workflow = normalizeSavedWorkflow({
    ...
    source: {
      ...
      edithVersion: cleanText(run.edithVersion, 40),
    },
    start: urlScope,
    parameters,
    steps,
...
export function compileWorkflowFromDemonstration(input, options = {}) {
  ...
  const workflow = normalizeSavedWorkflow({
    ...
    createdAt: ts,
    updatedAt: ts,
    source: { runId: '', edithVersion: cleanText(input?.edithVersion, 40) },
    start,
    parameters,
    steps,
    stats: {
...
export function normalizeSavedWorkflow(input, options = {}) {
  ...
  return {
    ...
    source: {
      ...
      edithVersion: cleanText(input?.source?.edithVersion, 40),
    },
    start,
    parameters,
    steps,
