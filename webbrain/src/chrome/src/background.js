...
/**
 * EDITH Service Worker (Background Script)
...
Promise.all([
  ...
  sweepOpfsSwapFiles().then(({ removed, bytes }) => {
    if (removed > 0) {
      console.info(`[EDITH] Reclaimed ${removed} orphaned OPFS swap file(s), ${(bytes / 1024 ** 3).toFixed(2)} GB.`);
    }
  }),
]).catch((error) => {
  console.warn('[EDITH] Apocalypse Mode startup work could not be restored:', error);
});
...
const CONTEXT_MENU_ASK_SELECTION_ID = 'edith-ask-selection';
const CONTEXT_MENU_OPEN_CHAT_ID = 'edith-selection-open-chat';
const CONTEXT_MENU_OPEN_PDF_VIEWER_ID = 'edith-open-pdf-viewer';
const CONTEXT_MENU_ACTION_PREFIX = 'edith-selection-action-';
const CONTEXT_MENU_TRANSLATE_ID = 'edith-selection-translate';
const CONTEXT_MENU_TRANSLATE_PREFIX = 'edith-selection-translate-';
const CONTEXT_MENU_GENERIC_ASK_ID = 'edith-selection-generic-ask';
...
function reportPdfMimeHandlerSyncFailure(error) {
  console.warn('[EDITH] Could not synchronize the native PDF MIME handler option:', error);
}
...
async function createContextMenus() {
  ...
  const create = (item) => {
    chrome.contextMenus.create(item, () => {
      ...
      if (err && !/duplicate/i.test(String(err.message || err))) {
        console.warn('[EDITH] Failed to create context menu:', err.message || err);
      }
  ...
  chrome.contextMenus.removeAll(() => {
    ...
    create({ id: 'edith-selection-separator-1', parentId: CONTEXT_MENU_ASK_SELECTION_ID, type: 'separator', contexts: ['selection'] });
    ...
    create({ id: 'edith-selection-separator-2', parentId: CONTEXT_MENU_ASK_SELECTION_ID, type: 'separator', contexts: ['selection'] });
...
function scheduleUserMemoryExtractionDrain(delayMs = USER_MEMORY_EXTRACTION_DELAY_MS) {
  ...
  userMemoryExtractionTimer = setTimeout(() => {
    ...
    drainUserMemoryExtractionQueue().catch((error) => {
      console.warn('[EDITH] user-memory extraction failed:', error);
    });
  }, delayMs);
}
...
function enqueueUserMemoryExtractionAfterTurn(payload) {
  queueMicrotask(() => {
    enqueueUserMemoryExtraction(payload).catch((error) => {
      console.warn('[EDITH] failed to enqueue user-memory extraction:', error);
    });
  });
}
...
async function loadCustomSkills() {
  ...
  if (retainedSkills.length !== storedSkills.length) {
    try {
      ...
      console.warn('[EDITH] Retired packaged skills could not be removed', error);
    }
  }
  ...
  try {
    ...
    console.warn('[EDITH] Default skills could not be loaded', e);
  }
  try {
    ...
    console.warn('[EDITH] Packaged skills could not be refreshed', e);
  }
...
async function showFirstInstallGuide(details) {
  ...
  await chrome.storage.local.set({ pinCoachmarkPending: true }).catch((error) => {
    console.warn('[EDITH] Could not prepare the first-open pin coachmark:', error);
  });
  try {
    ...
    console.warn('[EDITH] Could not open the first-install pinning guide:', error);
  }
}
...
chrome.runtime.onInstalled.addListener(async (details) => {
  ...
  console.log('[EDITH] Extension installed, providers loaded.');
});
...
chrome.storage.onChanged.addListener((changes, areaName) => {
  ...
  if (changes.providers || changes.activeProvider || changes.helpImproveEDITH) providerManager.load().catch(() => {});
  if (changes.edithCloudBridgeEnabled || changes.edithCloudBridgeUrl) {
  ...
  if (shouldClearUserMemoryExtractionQueueForChanges(changes)) {
    clearUserMemoryExtractionQueue().catch((error) => {
      console.warn('[EDITH] failed to clear user-memory extraction queue:', error);
    });
  }
  ...
  if (changes.capsolverApiKey || changes.captchaSolverEnabled) {
    loadCaptchaSolver()
      ...
      .catch((error) => console.warn('[EDITH] CapSolver setting could not be refreshed', error));
  }
...
async function runApocalypseDownloadPass() {
  ...
  try {
    ...
    try {
      ...
      console.warn('[EDITH] Failed to re-arm the Apocalypse archive download:', scheduleError);
    }
  }
}
...
chrome.alarms.onAlarm.addListener((alarm) => {
  if (alarm?.name === APOCALYPSE_DOWNLOAD_ALARM) {
    ...
    runApocalypseDownloadPass().catch((error) => {
      console.warn('[EDITH] Apocalypse Mode archive download failed:', error);
    }).finally(releaseKeepalive);
  } else if (alarm?.name === APOCALYPSE_UPDATE_ALARM) {
    apocalypseController.checkForUpdates().catch((error) => {
      console.warn('[EDITH] Apocalypse Mode update check failed:', error);
    });
  }
});
...
//
// We use a per-window "EDITH" tab group to keep an active sidebar session
...
// Per-window EDITH group ID. windowId -> tabGroups groupId.
const edithGroupByWindow = new Map();
const WB_GROUPS_KEY = 'edithGroupByWindow';

async function loadEDITHGroups() {
    ...
    if (Array.isArray(arr)) {
      ...
      for (const [windowId, groupId] of arr) {
        try {
          ...
          edithGroupByWindow.set(windowId, groupId);
        } catch { /* group gone, skip */ }
      }
    }
  } catch { /* session storage unavailable */ }
}
function saveEDITHGroups() {
  chrome.storage.session?.set({
    [WB_GROUPS_KEY]: Array.from(edithGroupByWindow.entries()),
  }).catch(() => {});
}
loadEDITHGroups();
...
//     alive for `open()`.
// We do NOT have a "tab left the EDITH group → disable panel" path,
 ...
 * EDITH group AND that `tab` is in it. Returns the group ID, or -1 when
 * disabled, unsupported, or failed.
 */
async function ensureEDITHGroup(tab) {
  if (!chrome.tabGroups || !tab?.id || tab.windowId == null) return -1;
  if (!await shouldAutoGroupTabs(chrome.storage.local)) return -1;
  try {
    let groupId = edithGroupByWindow.get(tab.windowId);
    ...
    if (groupId != null) {
      try {
        ...
        groupId = null;
        edithGroupByWindow.delete(tab.windowId);
        saveEDITHGroups();
      }
    }

    if (groupId == null) {
      // Always create a FRESH EDITH group for this window, even if the
      ...
      // it to "EDITH" — surprising for users who had a "Dev" or
      ...
      try {
        await chrome.tabGroups.update(groupId, {
          title: 'EDITH', color: 'blue', collapsed: false,
        });
      } catch { /* ignore styling failure */ }
      edithGroupByWindow.set(tab.windowId, groupId);
      saveEDITHGroups();
...
// the same panel visibility and EDITH group model.
...
// "Stop EDITH" floating button. The chat / chat_stream / continue
...
const BADGE_SUBSCRIBE_ERROR_RE = /(Subscribe for more usage|Upgrade to EDITH Plus):\s*(https?:\/\/\S+)/i;
...
function launchDetachedRun(action, msg, sender) {
  ...
  task.catch((error) => {
    ...
    console.warn(`[EDITH] detached ${action} run failed:`, error);
  }).finally(() => {
...
chrome.action.onClicked.addListener((tab) => {
  ...
  // gesture window for sidePanel.open, but ensureEDITHGroup doesn't
  // need it.
  ensureEDITHGroup(tab).catch(() => {});
});
...
// User ungrouped (or Chrome auto-collapsed) the EDITH group entirely.
...
chrome.tabGroups?.onRemoved?.addListener?.((group) => {
  for (const [windowId, gid] of edithGroupByWindow) {
    if (gid === group.id) {
      edithGroupByWindow.delete(windowId);
      saveEDITHGroups();
      break;
    }
  }
...
chrome.windows?.onRemoved?.addListener?.((windowId) => {
  if (edithGroupByWindow.has(windowId)) {
    edithGroupByWindow.delete(windowId);
    saveEDITHGroups();
  }
});
...
globalThis.__edithLastNav = lastNavByTab;
...
globalThis.__edithApiRequests = apiRequestsByTab;
globalThis.__edithApiRequestReplay = apiRequestReplayById;
  ...
  await new Promise((resolve) => {
    try {
      chrome.notifications.create({
        ...
        title: `EDITH — ${success ? 'Task finished' : 'Task needs attention'}`,
        message,
        silent: true,
      }, (notificationId) => {
  ...
  switch (msg.action) {
    ...
    case 'start_teacher_mode': {
      ...
      const result = await teacherRunInterlock.start(tabId, {
        name: msg.name,
        url: tab?.url,
        edithVersion: chrome.runtime.getManifest().version,
      });
      ...
      try {
        ...
        if (runCaptureState) {
          try {
            ...
            console.warn('[EDITH] trailing run capture failed to finish:', error);
    ...
    case 'set_help_improve_preference': {
      ...
      const stored = await chrome.storage.local.get('helpImproveEDITH');
      const previousEnabled = stored.helpImproveEDITH !== false;
      await chrome.storage.local.set({ helpImproveEDITH: msg.enabled });
      try {
        await providerManager.load();
      } catch (error) {
        if (previousEnabled !== msg.enabled) {
          await chrome.storage.local.set({ helpImproveEDITH: previousEnabled }).catch(() => {});
        }
        throw error;
      }
...
chrome.commands.onCommand.addListener(async (command) => {
  ...
  uiScaleCommandQueue = uiScaleCommandQueue.then(async () => {
    ...
    console.error('[EDITH] failed to update UI scale:', command, error);
  });
