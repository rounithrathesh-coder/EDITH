...
const CLOUD_RUN_STORAGE_KEY = 'edithCloudRunSnapshots';
...
export function normalizeCloudBridgeUrl(value = DEFAULT_CLOUD_BRIDGE_URL) {
  ...
  if (url.protocol !== 'ws:' || !['127.0.0.1', 'localhost', '::1', '[::1]'].includes(host)) {
    throw new Error('EDITH cloud bridge URL must use ws:// on localhost.');
  }
  return url.href;
}
...
export function createCloudRunController({
      ...
      for (const row of rows) {
        ...
        if (!TERMINAL_STATUSES.has(restored.status)) {
          ...
          restored.error = restored.status === 'aborted'
            ? 'Run aborted when the EDITH service worker restarted.'
            : 'Run interrupted when the EDITH service worker restarted.';
  ...
  async function startRun(msg = {}) {
    ...
    if (startingTabs.has(tabId) || agent.isRunning(tabId)) {
      throw new Error(`Tab ${tabId} already has an active EDITH run.`);
    }
        ...
        if (run.capture === 'video') {
          try {
            ...
            const recording = await startRecording(tabId, {
              ...
              mic: false,
              showBanner: false,
              filename: `edith-ci-${run.runId}.webm`,
            });
        ...
        if (recordingId) {
          try {
            ...
            pushUpdate(run, 'artifact', {
              kind: 'video',
              filename: capture.filename || `edith-ci-${run.runId}.webm`,
            });
          } catch (captureError) {
            pushUpdate(run, 'capture_error', {
              kind: 'video',
  ...
  async function respond(msg = {}) {
    ...
    if (!agent.submitClarifyResponse(run.tabId, clarifyId, answer, 'cloud_api')) {
      throw cloudRunError('Clarification is no longer available in the active EDITH run.', 409);
    }
    run.status = 'running';
    run.pendingInput = null;
    run.error = '';
  ...
  async function syncBridge() {
    const stored = await api.storage.local.get(['edithCloudBridgeEnabled', 'edithCloudBridgeUrl']);
    if (!stored.edithCloudBridgeEnabled) return stopBridge().catch(() => ({ enabled: false, connected: false }));
    return startBridge(stored.edithCloudBridgeUrl || DEFAULT_CLOUD_BRIDGE_URL);
  }
