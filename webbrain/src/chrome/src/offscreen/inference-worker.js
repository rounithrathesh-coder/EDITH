...
const WEBGPU_VISION_READY_MARKER_PREFIX = 'https://edith.one/.well-known/webgpu-vision-ready/';
...
function textReadyMarkerUrl(modelId, dtype) {
  ...
  return `https://edith.one/.well-known/webgpu-model-ready/${key}`;
}
...
function assertToolCapableTextRuntime(runtime, modelId) {
  ...
  throw new Error(`${modelId} is not compatible with EDITH: custom repositories must provide a chat template that accepts tools.`);
}
