...
const EDITH_CLOUD_PROVIDER_ID = 'edith_cloud';
const EDITH_CLOUD_PROVIDER_LABEL = 'EDITH Compass';
...
const EDITH_CLOUD_CONTEXT_WINDOW = 1000000;
const EDITH_CLOUD_LEGACY_CONTEXT_WINDOW = 256000;
const EDITH_DEVICE_GUID_KEY = 'edithDeviceGuid';
const HELP_IMPROVE_EDITH_KEY = 'helpImproveEDITH';
...
export class ProviderManager {
  ...
  async load() {
    const data = await chrome.storage.local.get([
      ...
      'activeProvider',
      'visionModel',
      EDITH_DEVICE_GUID_KEY,
      HELP_IMPROVE_EDITH_KEY,
      WEBGPU_VISION_ENABLED_KEY,
      WEBGPU_VISION_AUTO_SELECTED_KEY,
      WEBGPU_VISION_CONSENT_VERSION_KEY,
    ]);
    ...
    const legacyActiveProviderId = ['edith', 'openai_subscription'].includes(data.activeProvider)
      ? EDITH_CLOUD_PROVIDER_ID
      : data.activeProvider;
    ...
    for (const [id, config] of Object.entries(defaults)) {
      ...
      const configured = id !== EDITH_CLOUD_PROVIDER_ID && (
    ...
    for (const [id, config] of Object.entries(stored)) {
      if (!configs[id] && this._isSupportedProviderConfig(id, config)) {
        ...
        configs[id] = {
          ...config,
          configured: id !== EDITH_CLOUD_PROVIDER_ID && (config.configured === true || !hasConfiguredMarker),
        };
    ...
    if (this.activeProviderId === 'webgpu') {
      this.activeProviderId = EDITH_CLOUD_PROVIDER_ID;
      providerStateMigrated = true;
    }
    if (this.activeProviderId !== EDITH_CLOUD_PROVIDER_ID && configs[this.activeProviderId]?.configured !== true) {
      this.activeProviderId = EDITH_CLOUD_PROVIDER_ID;
      providerStateMigrated = true;
    }
  ...
  _defaultConfigs() {
    return {
      edith_cloud: {
        ...
        supportsVision: true,
        // EDITH Compass proxies to OpenRouter, whose upstream models
  ...
  _migrateStoredProviderConfigs(stored) {
    ...
    if (
      migrated[EDITH_CLOUD_PROVIDER_ID]
      && migrated[EDITH_CLOUD_PROVIDER_ID].label !== EDITH_CLOUD_PROVIDER_LABEL
    ) {
      migrated[EDITH_CLOUD_PROVIDER_ID] = {
        ...migrated[EDITH_CLOUD_PROVIDER_ID],
        label: EDITH_CLOUD_PROVIDER_LABEL,
    ...
    // Existing installs stored omitToolsWhenImagesPresent:true for EDITH
    ...
    if (migrated.edith_cloud?.omitToolsWhenImagesPresent) {
      migrated.edith_cloud = {
        ...migrated.edith_cloud,
        omitToolsWhenImagesPresent: false,
      };
    }
    if (Number(migrated.edith_cloud?.contextWindow) === EDITH_CLOUD_LEGACY_CONTEXT_WINDOW) {
      migrated.edith_cloud = {
        ...migrated.edith_cloud,
        contextWindow: EDITH_CLOUD_CONTEXT_WINDOW,
      };
  ...
  _canDuplicateProvider(id, config = this.providers.get(id)?.config) {
    return !!config && !config.duplicateOf && id !== EDITH_CLOUD_PROVIDER_ID && config.type !== 'webgpu';
  }
  ...
  /** Return the Chrome-only EDITH VL fallback, but only when explicitly enabled and ready. */
  ...
  async updateProvider(id, config, { markConfigured = true } = {}) {
    ...
    const merged = {
      ...current,
      ...updates,
      configured: id !== EDITH_CLOUD_PROVIDER_ID && (markConfigured || current.configured === true),
    };
  ...
  async removeDuplicateProvider(id) {
    ...
    if (previousActiveProviderId === id) {
      ...
      this.activeProviderId = source && (sourceId === EDITH_CLOUD_PROVIDER_ID || source.config?.configured === true)
        ? sourceId
        : EDITH_CLOUD_PROVIDER_ID;
    }
    try {
  ...
  async testTranscriptionProvider() {
    ...
    form.append('file', audioBlob, 'edith-connection-test.wav');
  ...
  async _getDeviceGuid(storedGuid) {
    ...
    try {
      await chrome.storage.local.set({ [EDITH_DEVICE_GUID_KEY]: guid });
    } catch (e) {
  ...
  async _guidFromMaterial(material) {
    const bytes = new TextEncoder().encode(`edith-device-v1:${material}`);
