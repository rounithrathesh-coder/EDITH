...
export const WEBGPU_VISION_MODEL_ID = 'edith-one/edith-vl-2-450M-onnx';
...
export function webgpuVisionReadyMarkerUrl(modelId = WEBGPU_VISION_MODEL_ID) {
  return `https://edith.one/.well-known/webgpu-vision-ready/v${WEBGPU_VISION_READY_MARKER_VERSION}/${encodeURIComponent(String(modelId || '').trim())}`;
}
...
export class WebGPUVisionProvider extends WebGPUOffscreenProvider {
  ...
  async chat(messages, options = {}) {
    ...
    const response = await this._dispatch({
      ...
      options: {
        maxTokens: options.maxTokens,
        ...(options.edithVisionProbe === true ? { visionProbe: true } : {}),
      },
