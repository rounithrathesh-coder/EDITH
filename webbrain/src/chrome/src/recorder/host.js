...
const RECORDING_SAFETY_ALARM_NAME = 'edith-recording-safety-cap';
...
function normalizeRecordingFilename(value) {
  ...
  const stem = filename.replace(/\.webm$/i, '').replace(/[. ]+$/g, '') || 'edith-recording';
...
function scheduleRecordingSafetyWatchdog(state = recordingState) {
  ...
  recordingSafetyTimeout = setTimeout(() => {
    stopRecordingForSafetyCap().catch((e) => {
      console.warn('[EDITH] recording safety cap failed:', e);
    });
  }, delay);
...
try {
  chrome.alarms?.onAlarm?.addListener?.((alarm) => {
    ...
    stopRecordingForSafetyCap().catch((e) => {
      console.warn('[EDITH] recording safety alarm failed:', e);
    });
  });
} catch {}
...
async function beforeRecordingFinalize(beforeFinalizeRecording) {
  ...
  try {
    ...
    console.warn('[EDITH] recording finalize provider preload failed:', e);
  }
}
...
export async function stopTabRecording(opts = {}) {
  ...
  const stamp = new Date()
    ...
    .replace(/T/, '_')
    .slice(0, 19);
  const filename = recordingState.filename || `edith-recording-${stamp}.webm`;
  ...
  if (wantTranscribe) {
    runTranscription({
      ...
      console.error('[EDITH] runTranscription crashed:', e);
    });
  }
