...
// These revisions describe controlled EDITH policy, never request content.
...
function systemPromptVariant(prompt) {
  ...
  if (text.startsWith("You are EDITH's private on-device chat assistant")) return 'standalone_webgpu';
  if (text.startsWith("You are EDITH's standalone chat assistant")) return 'standalone_chat';
  if (text.startsWith('You are EDITH, a helpful AI browser assistant running in Ask mode.')) return 'ask';

  let actTier = '';
  if (text.startsWith('You are EDITH, an AI browser agent running in Act mode. You can read web pages')) actTier = 'full';
  else if (text.startsWith('You are EDITH, an AI browser agent running in Act mode. You read web pages')) actTier = 'mid';
  else if (text.startsWith('You are EDITH, an AI browser agent. You control web pages through tools.')) actTier = 'compact';
  if (actTier) return text.includes('\nDEV MODE APPENDIX:\n') ? `dev_${actTier}` : `act_${actTier}`;

  if (text.startsWith('You are the planning subsystem for EDITH')) return 'planner';
  if (text.startsWith('You are the intent and compact planning subsystem for EDITH')) return 'planner_intent';
  if (text.startsWith('You classify how much of the active communication thread EDITH must read')) return 'read_scope';
  if (text.startsWith('You are EDITH producing a tool-free chat response')) return 'context_only';
  if (text.startsWith('You are EDITH on a forced terminal delivery turn')) return 'delivery_recovery';
