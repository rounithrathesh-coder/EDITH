const PRODUCTION_ORIGIN = 'https://edith.one';
...
function createSlot(url, markup) {
  ...
  slot.setAttribute('aria-label', 'EDITH preparedness bulletin');

  const frame = document.createElement('iframe');
  frame.className = 'apocalypse-comm-frame';
  frame.title = 'EDITH preparedness bulletin';
...
async function loadCommunication() {
  ...
  try {
    ...
    const allowedResponse = localRequest
      ...
      : responseUrl.protocol === 'https:' && ['edith.one', 'www.edith.one'].includes(responseUrl.hostname);
