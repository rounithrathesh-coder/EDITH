...
function hasNativeTextSpans(container) {
  for (const span of container?.querySelectorAll?.('span:not([data-edith-ocr])') || []) {
...
async function ocrCurrentPage() {
  ...
  try {
    ...
    setStatus(`OCR added ${rendered} text lines on page ${pageNumber}. Select the text to use EDITH actions.`, 'success');
  } catch (error) {
...
elements['zoom-out'].addEventListener('click', () => {
  ...
  rerender().catch(error => fallbackToNative(`EDITH could not zoom this PDF: ${error?.message || String(error)}`));
});
...
elements['fit-width'].addEventListener('click', () => {
  state.fitWidth = true;
  rerender().catch(error => fallbackToNative(`EDITH could not fit this PDF: ${error?.message || String(error)}`));
});
elements['rotate-page'].addEventListener('click', () => {
  cancelOcrRequest();
  state.rotation = (state.rotation + 90) % 360;
  state.ocrCache.clear();
  rerender().catch(error => fallbackToNative(`EDITH could not rotate this PDF: ${error?.message || String(error)}`));
});
...
elements['print-pdf'].addEventListener('click', () => {
  printPdf().catch(error => fallbackToNative(`EDITH could not print this PDF: ${error?.message || String(error)}`));
});
...
globalThis.addEventListener('resize', () => {
  ...
  state.resizeTimer = setTimeout(() => {
    rerender().catch(error => fallbackToNative(`EDITH could not resize this PDF: ${error?.message || String(error)}`));
  }, 120);
});
...
async function initialize() {
  ...
  if (!explicitViewer && !hasMimeHandler) {
    throw new Error('Chrome PDF MIME handler API is unavailable. Use the explicit EDITH PDF viewer entry instead.');
  }
  if (!explicitViewer) {
  ...
  if (!streamInfo?.streamUrl || !Number.isInteger(streamInfo.tabId)) {
    throw new Error('No readable PDF stream was provided. Open an online PDF or use the explicit EDITH PDF viewer link.');
  }
  state.streamInfo = streamInfo;
  if (streamInfo.embedded === true) document.body.dataset.embedded = 'true';
  elements['pdf-title'].textContent = String(streamInfo.originalUrl || 'EDITH PDF');
  globalThis.__edithSelectionShortcutConfig = {
...
initialize().catch(error => {
  fallbackToNative(`EDITH could not render this PDF: ${error?.message || String(error)}`);
});
