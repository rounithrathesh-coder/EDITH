...
(async function initOnboarding() {
  ...
  const stored = await chrome.storage.local.get(['onboardingComplete', 'helpImproveEDITH']);
...
async function parseSlashCommands(text, tabId = currentTabId, options = {}) {
  ...
  if (command.value === '/export' && action === 'conversation') {
    ...
    const edithVersion = chrome.runtime.getManifest().version || 'unknown';
    ...
    chrome.runtime.sendMessage(
      ...
      (response) => {
        if (chrome.runtime.lastError) {
          ...
          reject(new Error(`No response from EDITH background for "${action}". The background script may have restarted or crashed; reload the sidebar/extension and check the extension console for the original error.`));
