# EDITH Firefox Extension — Architecture

> Version 36.0.4 · Manifest V2 · Background Page
...
uses an explicit EDITH PDF viewer context-menu entry when the user chooses
...
EDITH Compass, nine local endpoints, Azure OpenAI, AWS Bedrock, Anthropic, and
...
| No arbitrary-path/CDP upload | Cannot attach an arbitrary local path silently | Use a prior `downloadId` re-fetch or EDITH's user file picker |
