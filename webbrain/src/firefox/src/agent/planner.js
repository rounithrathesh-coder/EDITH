...
export const PLANNER_SYSTEM_PROMPT = `You are the planning subsystem for EDITH, a browser automation agent. Given the user's task and current page context, output ONLY a single JSON object (no markdown fences, no commentary outside the JSON).
...
- allows_app_state_tool_evidence is true only when the requested work itself is reading/updating EDITH scratchpad or progress ledger (not incidental bookkeeping).
...
- For execute and plan_only requests, list 2–8 concrete steps. For respond and clarify, steps may be empty. Name real tools from this catalog when relevant:
  read: get_accessibility_tree, read_page, extract_data, fetch_url, research_url, inspect_viewport
...
export const PLANNER_INTENT_SYSTEM_PROMPT = `You are the intent and compact planning subsystem for EDITH, a browser automation agent. Output ONLY one JSON object:
{
  "request_kind": "execute" | "respond" | "plan_only" | "clarify",
  "site_job": null | "exact app-provided site workflow job id",
  "conditional_site_job": null | "edit-file-and-commit",
...
const PLANNER_RESUME_RULE = '\n- This run is an app-owned scheduled continuation. Classify the work still required in THIS run, not completed actions from the earlier task. First inspect the external event. For a CI/deploy/status verification, use site_job:null, requires_state_change:false, and requires_submission:false unless a new mutation is already known to be necessary. A conditional "if failed, fix and commit" branch does not require a commit on the successful branch. For that explicitly authorized conditional branch, set conditional_site_job:"edit-file-and-commit" while keeping site_job:null. EDITH will activate its mutation and exact commit-verification contract before any editor change. Use conditional_site_job:null when no such branch is authorized. Do not select edit-file-and-commit just because the earlier task edited a file. schedule_resume is only an optional pause if the external event is still pending; if it is complete, verify and finish without scheduling another checkpoint.';
...
export const READ_SCOPE_SYSTEM_PROMPT = `You classify how much of the active communication thread EDITH must read before answering. Output ONLY one JSON object:
{"read_scope":"complete_thread"|"current_message"|"visible_page"|"none"}
