...
/**
 * EDITH Background Script (Firefox)
...
Promise.all([
  ...
  console.warn('[EDITH] Apocalypse Mode schedules could not be restored:', error);
});
...
async function createContextMenus() {
    ...
    try {
      ...
      Promise.resolve(result).catch((e) => {
        if (!/duplicate/i.test(String(e?.message || e))) {
          console.warn('[EDITH] Failed to create context menu:', e?.message || e);
        }
      });
    } catch (e) {
      if (!/duplicate/i.test(String(e?.message || e))) {
  ...
  const create = () => {
    ...
    createItem({ id: 'edith-selection-separator-1', parentId: CONTEXT_MENU_ASK_SELECTION_ID, type: 'separator', contexts: ['selection'] });
    ...
    createItem({ id: 'edith-selection-separator-2', parentId: CONTEXT_MENU_ASK_SELECTION_ID, type: 'separator', contexts: ['selection'] });
...
// ────────────────────────────────────────────────────────────────────────
// Tab grouping (visual scope for a EDITH session)
//
...
// action, the source tab joins (or seeds) a colored "EDITH" tab group
// for that window. Internal helper tabs and target=_blank redirects auto-join
// the same group via agent.js's `_addToEDITHGroup`.
//
...
const edithGroupByWindow = new Map(); // windowId -> tabGroups groupId
 ...
 * "EDITH" group AND that `tab` is in it. Always creates a fresh group
...
// EDITH grouping as browser-action opens.
...
function openSidebarForContextMenu(tab) {
  ...
  if (tab?.id) ensureEDITHGroup(tab).catch(() => {});
}
...
// in the EDITH group so the colored label appears immediately.
...
// EDITH" floating button. The chat / chat_stream / continue handlers
  ...
  switch (msg.action) {
    ...
    case 'start_teacher_mode': {
      ...
      const result = await teacherRunInterlock.start(tabId, {
        name: msg.name,
        url: tab?.url,
        edithVersion: browser.runtime.getManifest().version,
      });
    ...
    case 'set_help_improve_preference': {
      ...
      const stored = await browser.storage.local.get('helpImproveEDITH');
      const previousEnabled = stored.helpImproveEDITH !== false;
      await browser.storage.local.set({ helpImproveEDITH: msg.enabled });
      try {
        await providerManager.load();
      } catch (error) {
        if (previousEnabled !== msg.enabled) {
          await browser.storage.local.set({ helpImproveEDITH: previousEnabled }).catch(() => {});
        }
        throw error;
      }
...
browser.commands.onCommand.addListener(async (command, tab) => {
  ...
  try {
    ...
    console.error('[EDITH] failed to dispatch command:', command, err);
  }
});
