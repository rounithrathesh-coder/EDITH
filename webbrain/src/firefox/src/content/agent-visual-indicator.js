/**
 * EDITH Agent Visual Indicator (content script)
 *
 * Renders an animated purple inset glow around the page viewport while
 * the agent is operating on this tab, a "Stop EDITH" floating button,
 * and a large EDITH-purple cursor/outline on the element currently
 * being acted on. The base indicator follows Anthropic's Claude-for-Chrome
 * extension pattern, recolored for EDITH's accent (#6c63ff).
 *
...
(function () {
  if (window.__edithAgentIndicatorInjected) return;
  window.__edithAgentIndicatorInjected = true;

  const TARGET_CURSOR_TTL_MS = 3000;
  // The background refreshes this lease every 20 seconds. If its worker
  // disappears before the normal HIDE message, the page-owned UI cleans
  // itself up instead of leaving a permanent glow and Stop button.
  ...
  function injectStyles() {
    if (document.getElementById('edith-agent-styles')) return;
    const style = document.createElement('style');
    style.id = 'edith-agent-styles';
    style.textContent = `
      ...
      @keyframes edith-target-pop {
        0% { opacity: 0; }
        100% { opacity: 1; }
      }

      @keyframes edith-target-ring {
        0%, 100% {
          box-shadow:
  ...
  function createStopButton() {
    ...
    container.id = 'edith-agent-stop-container';
    ...
    button.id = 'edith-agent-stop-button';
    button.type = 'button';
    button.innerHTML = `
      ...
      <span style="vertical-align: middle;">Stop EDITH</span>
    `;
  ...
  function createTargetOutline() {
    const el = document.createElement('div');
    el.id = 'edith-agent-target-outline';
    el.style.cssText = `
      position: fixed;
      pointer-events: none;
      z-index: 2147483645;
      ...
      transition:
        ...
        width 0.18s ease,
        height 0.18s ease;
      animation: edith-target-ring 1.3s ease-in-out infinite;
    `;
    return el;
  }
  ...
  function createTargetCursor() {
    const el = document.createElement('div');
    el.id = 'edith-agent-target-cursor';
    ...
    el.style.cssText = `
      ...
      animation: edith-target-pop 0.18s ease-out;
  ...
  function show() {
    ...
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        ...
        const btn = stopContainerEl?.querySelector('#edith-agent-stop-button');
        if (btn) {
  ...
  window.__edithAgentIndicator = { showTarget, showTargetRect, hideTarget: hideTargetCursor };
