/**
 * EDITH Content Script
 * Injected into every page — handles page reading and DOM actions.
 */

(() => {
  // Prevent double-injection
  if (window.__edith_injected) return;
  window.__edith_injected = true;
  ...
  function showAgentWorkingTarget(el, source = 'interaction') {
    try {
      window.__edithAgentIndicator?.showTarget?.(el, source);
    } catch {}
  }
   ...
   * file directly (or presents EDITH's own picker when no downloadId is
  ...
  function clickWithoutNativeFilePicker(runClick, settleMs = FILE_PICKER_GUARD_SETTLE_MS) {
    ...
    const installPageShowPickerGuard = () => {
      ...
      if (!root) return () => {};
      const guardAttr = 'data-edith-file-picker-guard';
      const blockedAttr = 'data-edith-file-picker-blocked';
      const blockedEvent = 'edith:file-picker-guard-blocked';
      const armPageGuard = () => {
      ...
      return (disarmPageGuard = true) => {
        if (disarmPageGuard) {
          document.dispatchEvent(new Event('edith:file-picker-guard-disarm'));
        }
  ...
  function filePickerBlockedResponse(blocked, label = '') {
    ...
    const guidance = selector
      ? `Call upload_file({selector: ${JSON.stringify(selector)}, downloadId: N}) directly; it attaches the downloaded file without opening an OS dialog. If there is no downloadId, call upload_file({selector: ${JSON.stringify(selector)}}) to use EDITH's own picker.`
    ...
    try {
      ...
      return {
        ...
        warning: verified
          ...
          : 'window.find reported a match, but EDITH could not verify a visible current selection in the top document (for example, the active match may be inside a frame while an older top-document selection remains). Do not claim it is visibly highlighted. The browser Find UI was not opened.',
      };
    } catch (error) {
  ...
  function _waitForRichTextToolbarFocusedChildFrame(params = {}) {
    ...
    return new Promise(resolve => {
      ...
      const onMessage = event => {
        if (event?.data?.__edithFocusedFrameToken !== token) return;
  ...
  function _announceRichTextToolbarFocusedChildFrame(params = {}) {
    ...
    try {
      window.parent.postMessage({ __edithFocusedFrameToken: token }, '*');
  ...
  function attentionFlashCaptureIcons() {
    try {
      ...
      if (!attentionFlashIconLinks.length && !attentionFlashFallbackEl) {
        ...
        attentionFlashFallbackEl.setAttribute('data-edith-attention', '1');
