...
(function () {
  'use strict';
  if (globalThis.__edith_redaction_regions_injected) return;
  globalThis.__edith_redaction_regions_injected = true;
  ...
  function waitForExactChildFrameRect(params) {
    ...
    return new Promise(resolve => {
      ...
      const onMessage = event => {
        if (event?.data?.__edithExactFrameRectToken !== token) return;
  ...
  function announceExactChildFrame(params) {
    ...
    try {
      window.parent.postMessage(
        { __edithExactFrameRectToken: token },
        parentOrigin || '*',
      );
