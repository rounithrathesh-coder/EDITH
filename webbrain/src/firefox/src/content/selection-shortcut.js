/**
 * Floating EDITH shortcut for selected page text.
 *
 ...
 */
(function () {
  const shortcutConfig = globalThis.__edithSelectionShortcutConfig;
  if ((window.top !== window && shortcutConfig?.allowNestedFrame !== true)
      || window.__edithSelectionShortcutInjected) return;
  window.__edithSelectionShortcutInjected = true;
...
host.lang = localization.locale;
  ...
  function ensureSurface() {
    ...
    host.id = 'edith-selection-shortcut-host';
    ...
    shadow.innerHTML = `
      ...
      <button class="shortcut" type="button" aria-label="Ask EDITH about this" title="Ask EDITH about this" hidden>
        <span class="shortcut-icon" aria-hidden="true">?</span>
      </button>
      <div class="popup" role="dialog" aria-label="Ask EDITH about this" hidden>
        <div class="main-view">
          <div class="actions">
          ...
          <div class="question-wrap">
            <textarea maxlength="2000" rows="3" aria-label="Ask EDITH a question" placeholder="Ask EDITH…"></textarea>
  ...
  async function submitSelection(action, customQuestion = '', language = '') {
    ...
    try {
      ...
      if (response.requiresManualOpen) showToast(localization?.strings.sentManual || 'Sent to EDITH. Open the sidebar if it does not start.');
    } catch {
      showToast(localization?.strings.sendFailed || 'Could not send to EDITH. Try the right-click menu instead.');
    } finally {
      submitting = false;
    }
  }
  ...
  window.__edithSelectionShortcut = {
