...
/**
 * Network & download tools for the EDITH agent (Firefox).
 *
...
function apiReplayOptionsForFetch(rawUrl, opts = {}, ctx = {}) {
  ...
  const replay = globalThis.__edithApiRequestReplay?.get(String(replayRequestId));
  if (!replay) {
...
function cancelPageReplay(requestId) {
  const registry = globalThis.__edithNetworkReplays ||= new Map();
