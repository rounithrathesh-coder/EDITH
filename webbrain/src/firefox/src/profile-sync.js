...
export const PROFILE_SYNC_KEYS = {
  enabled: 'profileSyncEnabled', token: 'profileSyncToken', deviceGuid: 'edithDeviceGuid',
...
const DEFAULT_PORTABLE_ACTIVE_PROVIDER = 'edith_cloud';
...
export class ProfileSyncManager {
  ...
  async authStatus(challengeId, verifier) { const q = new URLSearchParams({ challenge_id: challengeId }); const r = await this.request(`/auth/status?${q}`, { headers: { 'X-EDITH-Sync-Verifier': verifier } }); if (r.body.token) await this.storage.set({ [PROFILE_SYNC_KEYS.token]: r.body.token }); return r.body; }
