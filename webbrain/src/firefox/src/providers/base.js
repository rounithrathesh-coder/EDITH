...
export class BaseLLMProvider {
  ...
  _mapMessages(messages) {
    const sanitized = (Array.isArray(messages) ? messages : []).map((message) => {
      if (!message || typeof message !== 'object' || (
        !Object.hasOwn(message, 'edithPlannerClarification')
        && !Object.hasOwn(message, 'edithAppOwned')
        && !Object.hasOwn(message, 'edithAppOwnedKind')
        && !Object.hasOwn(message, 'edithSelectionScopeRestored')
      )) {
        return message;
      }
      const {
        edithPlannerClarification: _plannerClarification,
        edithAppOwned: _appOwned,
        edithAppOwnedKind: _appOwnedKind,
        edithSelectionScopeRestored: _selectionScopeRestored,
        ...providerMessage
      } = message;
