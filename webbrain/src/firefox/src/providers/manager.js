...
export class ProviderManager {
   ...
   */
  async load() {
    const data = await browser.storage.local.get(['providers', 'activeProvider', EDITH_DEVICE_GUID_KEY, HELP_IMPROVE_EDITH_KEY]);
  ...
  async _getDeviceGuid(storedGuid) {
    ...
    try {
      await browser.storage.local.set({ [EDITH_DEVICE_GUID_KEY]: guid });
    } catch (e) {
