...
export function shouldUseOpenAIResponsesApi(config = {}) {
  ...
  // EDITH's OpenCode Zen provider previously forced Chat Completions for all Zen models (404 for Responses models).
...
export function plannerRequestBody(config = {}, {
  ...
  if (schema && STRUCTURED_OUTPUT_PROVIDER_NAMES.has(providerName)) {
    body.response_format = {
      type: 'json_schema',
      json_schema: {
        name: String(schemaName || 'edith_planner').replace(/[^a-zA-Z0-9_-]/g, '_').slice(0, 64),
        strict: true,
        schema,
      },
    };
