...
export function isBackgroundConnectionError(error) {
  ...
  return /Could not establish connection|Receiving end does not exist|Extension context invalidated|message (?:channel|port) closed|No response from EDITH background|background script may have restarted|extension connection was lost/i.test(message);
}
