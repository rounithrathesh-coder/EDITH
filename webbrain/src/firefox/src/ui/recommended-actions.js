...
function edithTweetRunOptions(postText) {
  const exactPost = String(postText || '').trim();
  return {
    id: 'tweet-edith',
    skipPlanner: true,
    tool: 'navigate',
    summary: 'Publish the reviewed localized EDITH post exactly as supplied.',
    steps: [
...
function edithLinkedInRunOptions(postText) {
  const exactPost = String(postText || '').trim();
  return {
    id: 'post-edith-linkedin',
    skipPlanner: true,
    tool: 'navigate',
    summary: 'Publish the reviewed localized EDITH post on LinkedIn exactly as supplied.',
    steps: [
...
export function buildRecommendedActions(pageInfo = {}, options = {}) {
  ...
  const actions = [];
  const edithPostText = t('sp.recommended.tweet.text');
  const edithPromotionVariant = options.edithPromotionVariant === 'linkedin' ? 'linkedin' : 'x';

  const edithPromotion = edithPromotionVariant === 'linkedin'
    ? {
      id: 'post-edith-linkedin',
      label: t('sp.recommended.linkedin.label'),
      prompt: t('sp.recommended.linkedin.prompt', { post: edithPostText }),
      mode: 'act',
      runOptions: edithLinkedInRunOptions(edithPostText),
    }
    : {
      id: 'tweet-edith',
      label: t('sp.recommended.tweet.label'),
      prompt: t('sp.recommended.tweet.prompt', { post: edithPostText }),
      mode: 'act',
      runOptions: edithTweetRunOptions(edithPostText),
    };
...
return { found, loop: state.video?.loop === true, intervalSet: Boolean(state.timer), paused: state.video?.paused ?? null, duration: state.video?.duration ?? null };`,
  ...
  addUnique(actions, edithPromotion);
