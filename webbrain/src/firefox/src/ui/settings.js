/**
 * EDITH Settings Page — provider configuration + display settings.
 */
...
const EASY_CLI_PROXY_GUIDE_URL = 'https://edith.one/docs/easy-cli-proxy/';
...
function edithSubscribeUrl(deviceGuid) {
  const url = new URL(EDITH_SUBSCRIBE_URL);
  if (deviceGuid) {
...
function edithAccountUrl(deviceGuid) {
  const url = new URL(EDITH_ACCOUNT_URL);
  if (deviceGuid) {
    url.searchParams.set('client_reference_id', deviceGuid);
  }
  return url.toString();
...
async function init() {
  // Migration: the old auth.edith.one sign-in stored a bearer token and
  ...
  if (autoGroupTabsToggle) autoGroupTabsToggle.checked = stored[AUTO_GROUP_TABS_KEY] === true;
  if (helpImproveToggle) helpImproveToggle.checked = stored.helpImproveEDITH !== false; // on by default
...
helpImproveToggle?.addEventListener('change', async () => {
  await browser.storage.local.set({ helpImproveEDITH: helpImproveToggle.checked }).catch(() => {});
});
...
if (btnExportUserMemory) {
  btnExportUserMemory.addEventListener('click', async () => {
    ...
    a.href = url;
    a.download = `edith-user-memory-${Date.now()}.json`;
...
function supportsProviderCompatibilityControls(id, config = {}) {
  return id !== 'edith_cloud' && ['openai', 'llamacpp', 'azure_openai'].includes(config.type);
}
...
function renderProviders() {
  ...
  for (const [id, definition] of Object.entries(providerConfigs)) {
    if (id === 'edith_cloud' || !Array.isArray(definition.fields)) continue;
  ...
  for (const [id, config] of entries) {
    ...
    const isConfigured = id !== 'edith_cloud' && config.configured === true;
    const definitionId = providerDefinitionId(id, config);
    ...
    const subscribeHref = id === 'edith_cloud' ? edithSubscribeUrl(config.deviceGuid) : '';
    const accountHref = id === 'edith_cloud' ? edithAccountUrl(config.deviceGuid) : '';
    const billingButton = id === 'edith_cloud'
    ...
    if (id === 'edith_cloud') {
      ...
      providerNote = `<div style="margin-top:10px;padding:10px 12px;border-radius:6px;
           ...
           ${t('st.providers.edith_data_use.body', { privacyLink, subscribeLink, accountLink })}
         </div>`;
    }
    ...
    const ollamaWarning = definitionId === 'ollama'
      ? `<aside class="provider-warning provider-ollama-warning" role="note"
           ...
           <a href="https://www.edith.one/blog/ollama-launch-handoff"
...
function providerIsActive(id, config) {
  return id !== 'edith_cloud' && config?.configured === true;
}
...
async function saveProvider(id, { showFlash = true, markConfigured = true } = {}) {
  ...
  if (providersData[id]) {
    ...
    if (markConfigured) providersData[id].configured = id !== 'edith_cloud';
  }
