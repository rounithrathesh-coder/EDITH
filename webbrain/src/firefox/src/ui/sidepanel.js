/**
 * EDITH Side Panel — Chat UI logic.
...
(async function initOnboarding() {
  const stored = await browser.storage.local.get(['onboardingComplete', 'helpImproveEDITH']);
  ...
  let persistedHelpImprove = stored.helpImproveEDITH !== false;
  ...
  async function scanLocalModels() {
    ...
    try {
      ...
      if (active === 'edith_cloud' && providers.edith_cloud?.enabled !== false) {
        showCloudReady();
        return;
      }
      ...
      if (choices.length > 0) {
        ...
        console.warn('[EDITH] onboarding local-model scan failed:', errors);
...
function buildRunScreenshotFilenames(saveAs, date = new Date()) {
  ...
  const stem = (requested.replace(/\.png$/i, '') || `edith-run-${runCaptureTimestamp(date)}`).slice(0, 170);
  return {
...
function buildRunRecordingFilename(saveAs) {
  ...
  const stem = requested.replace(/\.webm$/i, '').replace(/[. ]+$/g, '') || 'edith-recording';
...
let selectedProviderId = 'edith_cloud';
...
let edithPromotionHasAnimated = false;
...
function migrateLegacyEmptyStateFromRestoredChat(tabId, root = messagesEl) {
  ...
  if (Number.isFinite(numericTabId)) {
    ...
    void persistTabChat(numericTabId, migratedHtml, { allowHidden: true }).catch((error) => {
      console.warn('[EDITH] failed to persist restored empty-state migration:', error);
    });
  }
  return true;
}
...
async function persistChatHistorySnapshot(tabId, { refreshTabInfo = false } = {}) {
  ...
  await saveChatHistoryRecord({
    ...
    console.warn('[EDITH] failed to save chat history:', error);
  });
}
...
async function repairRestoredChatHistorySnapshot(tabId) {
  ...
  await repairChatHistoryRecordMessages(recordId, messages).catch((error) => {
    console.warn('[EDITH] failed to repair restored chat history:', error);
  });
}
...
async function resetChatHistoryStateForTab(tabId) {
  ...
  await Promise.all(Array.from(recordIdsToDelete).map((recordId) => (
    deleteChatHistoryRecord(recordId).catch((error) => {
      console.warn('[EDITH] failed to delete chat history:', error);
    })
  )));
...
async function refreshScheduledJobs({ tabId = null } = {}) {
  ...
  try {
    ...
    console.warn('[EDITH] failed to refresh scheduled jobs:', e);
    return [];
  }
}
...
function savedWorkflowDownloadFilename(name) {
  ...
  return `${stem}.edith-workflow.json`;
}
...
function requestSavedWorkflowFile(tabId) {
  ...
  input.type = 'file';
  input.accept = '.json,.edith-workflow.json,application/json';
...
async function init() {
  ...
  await loadProviders();
  await testConnection({ skipEDITHCloud: true });
    ...
    if (e.shiftKey) {
      try {
        ...
        if (response?.log?.length) {
          console.group('%c[EDITH Deep Verbose] %d entries', 'color:#7c3aed;font-weight:bold', response.log.length);
          ...
          console.log('%c[EDITH Deep Verbose] No entries yet — run a query first.', 'color:#7c3aed');
        }
      } catch (err) {
        console.error('[EDITH Deep Verbose] Failed to fetch debug log:', err);
      }
...
function createEdithPromotionIcon(actionId) {
  ...
  path.setAttribute('d', actionId === 'post-edith-linkedin'
...
function animateEdithPromotionOnce() {
  if (edithPromotionHasAnimated || recommendedActionsCollapsed) return;
  ...
  edithPromotionHasAnimated = true;
}
...
async function refreshRecommendedActions() {
  ...
  try {
    ...
    const edithPromotionVariant = Math.random() < 0.5 ? 'linkedin' : 'x';
    const actions = buildRecommendedActions(pageInfo, { max: 4, edithPromotionVariant });
    recommendedActionsListEl.replaceChildren();
    actions.forEach((action) => {
      ...
      if (EDITH_PROMOTION_ACTION_IDS.has(action.id)) {
        btn.classList.add('recommended-action-chip-promotion');
        btn.prepend(createEdithPromotionIcon(action.id));
      }
...
function appendProviderPickerOption(id, name, meta, iconProviderId = id) {
  ...
  // EDITH mark (and other brand chips) don't compete with the chrome.
...
async function loadProviders() {
  try {
    ...
    const cloudConfig = res.providers.edith_cloud || { label: 'EDITH Compass' };
    const cloudLabel = cloudConfig.label || 'EDITH Compass';
    const cloudGroup = document.createElement('optgroup');
    cloudGroup.label = t('sp.providers.no_setup_group');
    const cloudOption = document.createElement('option');
    cloudOption.value = 'edith_cloud';
    ...
    providerPickerLabelById.set('edith_cloud', cloudLabel);
    appendProviderPickerGroup(cloudGroup.label);
    appendProviderPickerOption('edith_cloud', cloudLabel, t('sp.providers.no_setup'));

    const configuredEntries = Object.entries(res.providers)
      .filter(([id, config]) => id !== 'edith_cloud' && config?.configured === true);
    ...
    const selectableProviderIds = new Set(['edith_cloud', ...configuredEntries.map(([id]) => id)]);
    selectedProviderId = selectableProviderIds.has(res.active) ? res.active : 'edith_cloud';
...
function isEDITHCloudProviderSelected() {
  return providerSelect?.value === 'edith_cloud';
}
...
async function testConnection(options = {}) {
  ...
  if (options.skipEDITHCloud && providerId === 'edith_cloud') {
...
function screenshotDownloadFilename(pageUrl = '', fullPage = false) {
  const prefix = screenshotFilenamePrefix(pageUrl) || 'edith';
...
async function parseSlashCommands(text, tabId = currentTabId, options = {}) {
  ...
  if (command.value === '/export' && action === 'config') {
    try {
      ...
      a.href = url;
      a.download = `edith-config-${Date.now()}.json`;
  ...
  if (command.value === '/export' && action === 'traces') {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `edith-traces-${Date.now()}.md`;
    document.body.appendChild(a);
    try {
      a.click();
    } finally {
  ...
  if (command.value === '/export' && action === 'conversation') {
    ...
    const edithVersion = browser.runtime.getManifest().version || 'unknown';
    let md = `# EDITH Conversation\n\n_Exported with EDITH v${edithVersion}_\n\n`;
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `edith-chat-${Date.now()}.md`;
    document.body.appendChild(a);
    try {
      a.click();
    } finally {
...
function handleAgentUpdateMessage(msg) {
  ...
  if (msg.type === 'scheduled_job') {
    handleScheduledJobEvent(msg.data, msg.tabId).catch((err) => {
      console.warn('[EDITH] failed to handle scheduled job event:', err);
    });
    return;
  }
...
function renderClarifyCard(data) {
  ...
  if (data.submitConfirmation) {
    ...
    qEl.textContent = String(data.question || `EDITH wants to submit this form on ${host}.`).slice(0, 600);
...
function submitPlanReview(card, tabId, planId, action, editedText) {
  ...
  const failureText = (error) => isBackgroundConnectionError(error)
    ? 'EDITH reloaded or the background worker stopped before this plan could be approved. Reload the sidebar and try again.'
    : expiredText();
...
// EDITH Compass returns a 402 with one trailing billing action. Keep the
// matcher narrow so ordinary subscription text is not converted into billing UI.
const SUBSCRIBE_ERROR_RE = /(Subscribe for more usage|Upgrade to EDITH Plus):\s*(https?:\/\/\S+)/i;
...
function scheduleMathRender() {
  ...
  _mathRenderTimer = setTimeout(() => {
    ...
    try {
      ...
      console.warn('[edith] math render failed:', e);
    }
  }, 50);
}
...
function formatBackgroundSendError(action, message) {
  if (String(message || '').trim() === `Unknown action: ${action}`) {
    return `EDITH's sidebar and background are out of sync. Reload EDITH from your browser's extension manager, reopen the sidebar, and try again.`;
  }
  if (isBackgroundConnectionError(message)) {
    return `EDITH extension connection was lost while sending "${action}". Reload the sidebar/extension and try again.`;
  }
  return message;
}
...
async function sendToBackground(action, data = {}) {
  ...
  if (response == null) {
    throw new Error(`No response from EDITH background for "${action}". The background script may have restarted or crashed; reload the sidebar/extension and check the Firefox extension console for the original error.`);
  }
  if (response?.error) {
...
function decodeStagedScreenshotMetadata(value, dataUrl, storedRecord = null) {
  try {
    ...
    return {
      kind: 'image',
      name: String(metadata.name || 'edith-screenshot.png').slice(0, 240),
      dataUrl,
