...
document.getElementById('btn-export').addEventListener('click', async () => {
  ...
  try {
    ...
    const payload = buildTraceExportPayload(entries, {
      sessionId,
      exportedAt: Date.now(),
      exportedByEDITHVersion: browser.runtime.getManifest().version || '',
    });
    ...
    a.download = isSession
      ? `edith-session-${safeFilenamePart(sessionId, 'session')}.json`
      : `edith-trace-${run.model || 'unknown'}-${run.runId}.json`;
