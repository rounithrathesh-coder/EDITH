# Vendored fflate runtime

- Package: `fflate`
- Version: `0.8.3`
- Upstream: <https://github.com/101arrowz/fflate>
- File: `esm/browser.js` from the published npm package, unmodified
- SHA-256: `b7ca4450b19559a1d50eb381adcee94b82449674be4cd17789d9beba7e6122a1`
- License: MIT (see `LICENSE`)

EDITH uses the streaming `Unzip` and `UnzipInflate` exports for bounded-memory
Emergency Box text-pack extraction. The dependency is bundled with the
extension; no executable code is fetched at runtime.
