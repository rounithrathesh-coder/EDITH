...
async function setupIsolatedContentHtml(page, html, browserKind) {
  ...
  const isolatedWorld = await session.send('Page.createIsolatedWorld', {
    ...
    worldName: `edith-${browserKind}-fixture`,
  });
...
async function setupSelectionShortcut(page, sourcePath, { enabled = true, requiresManualOpen = false, locale = 'en' } = {}) {
  ...
  await page.setContent(`<!doctype html>
    ...
    <p id="copy">Selected words near the viewport edge for EDITH.</p>
  ...
  const src = await readFile(sourcePath, 'utf-8');
  await page.addScriptTag({ content: src });
  await page.waitForFunction(() => typeof window.__edithSelectionShortcut?.getState === 'function');
}
...
async function selectFixtureText(page, selector = '#copy') {
  ...
  await page.waitForFunction(() => window.__edithSelectionShortcut.getState().shortcutVisible);
  await page.waitForTimeout(20);
  return page.evaluate(() => window.__edithSelectionShortcut.getState());
}
...
for (const [label, sourcePath, manualOpen] of [
  ...
  test(`${label}: selection shortcut localizes labels, direction, and fixed-action language`, async (page) => {
    ...
    await page.evaluate(() => window.__edithSelectionShortcut.submitPreset('explain'));
    ...
    await page.waitForFunction(() => window.__edithSelectionShortcut.getState().direction === 'rtl');
    const rtl = await page.evaluate(() => window.__edithSelectionShortcut.getState());
  ...
  test(`${label}: custom selection questions default to page context and retain a scoped choice`, async (page) => {
    ...
    await page.evaluate(() => {
      window.__edithSelectionShortcut.setIncludePageContext(false);
      return window.__edithSelectionShortcut.submitCustom('仅根据选中内容回答。');
    });
    await page.waitForFunction(() => window.__selectionMessages.length === 1);
    const submitted = await page.evaluate(() => window.__selectionMessages[0]);
    ...
    await page.waitForFunction(() => !window.__edithSelectionShortcut.getState().submitting);
    ...
    await page.evaluate(() => window.__edithSelectionShortcut.submitCustom('现在有哪些跨平台框架？'));
  ...
  test(`${label}: selection shortcut clamps to the viewport and supports keyboard dismissal`, async (page) => {
    ...
    if (state.shortcutLabel !== 'Ask EDITH about this'
    ...
    let popupState = await page.evaluate(() => window.__edithSelectionShortcut.getState());
    if (!popupState.popupVisible) throw new Error('popup did not open for the selected text');
    await page.keyboard.press('Escape');
    popupState = await page.evaluate(() => window.__edithSelectionShortcut.getState());
  ...
  test(`${label}: selection dialog contains page shortcuts and keeps the selected text highlighted`, async (page) => {
    ...
    const openState = await page.evaluate(() => window.__edithSelectionShortcut.getState());
    ...
    const typedState = await page.evaluate(() => ({
      surface: window.__edithSelectionShortcut.getState(),
    ...
    await page.keyboard.press('Escape');
    const closedState = await page.evaluate(() => window.__edithSelectionShortcut.getState());
    ...
    await page.waitForFunction(() => window.__edithSelectionShortcut.getState().popupVisible);
    const reopenedState = await page.evaluate(() => window.__edithSelectionShortcut.getState());
    await page.mouse.click(
  ...
  test(`${label}: selection shortcut submits once and dismisses before delivery`, async (page) => {
    ...
    await page.waitForFunction(() => window.__selectionMessages.length === 1);
    await page.evaluate(() => window.__edithSelectionShortcut.submitPreset('summarize'));
    const result = await page.evaluate(() => ({
      messages: window.__selectionMessages,
      state: window.__edithSelectionShortcut.getState(),
    }));
    ...
    await page.evaluate(() => window.__edithSelectionShortcut.submitCustom('   '));
    ...
    await page.evaluate(() => window.__edithSelectionShortcut.submitCustom('What is the point?'));
    ...
    const translateState = await page.evaluate(() => window.__edithSelectionShortcut.getState());
    ...
    const updatedLocaleState = await page.evaluate(() => window.__edithSelectionShortcut.getState());
    await page.mouse.click(
  ...
  test(`${label}: selection shortcut persists hiding and suppresses screenshot-time UI`, async (page) => {
    ...
    if (manualOpen) {
      await page.evaluate(() => window.__edithSelectionShortcut.submitPreset('summarize'));
      const toastState = await page.evaluate(() => window.__edithSelectionShortcut.getState());
    ...
    let state = await page.evaluate(() => window.__edithSelectionShortcut.getState());
    ...
    await selectFixtureText(page);
    await page.evaluate(() => window.__edithSelectionShortcut.hideShortcut());
    ...
    await page.waitForTimeout(20);
    state = await page.evaluate(() => window.__edithSelectionShortcut.getState());
...
const gmailThreadScopeFixture = `<!doctype html>
  ...
  <main id="active-thread" aria-label="A chat about EDITH and your work">
    <h1>A chat about EDITH and your work</h1>
...
test('Chrome Agent: modal auto-select ignores background/hidden clickables and keeps the exact target', async (page) => {
  ...
  const values = await page.evaluate(() => ({
    ...
    leakedTargetSlots: Object.keys(globalThis).filter((key) => key.startsWith('__edithAutoSelectTarget_')),
  }));
...
test('CDP type_text binds dispatch to the selector node approved by toolbar preflight', async (page) => {
  ...
  await page.evaluate(() => {
    const observer = new MutationObserver(records => {
      if (!records.some(record => record.attributeName === 'data-edith-dispatch-binding')) return;
      observer.disconnect();
      const previous = document.querySelector('.shared-target');
      const replacement = previous.cloneNode();
    ...
    observer.observe(document.querySelector('.shared-target'), {
      attributes: true,
      attributeFilter: ['data-edith-dispatch-binding'],
    });
  });
  ...
  const leakedMarkers = await page.locator('[data-edith-dispatch-binding]').count();
  ...
  for (const [deferral, scheduleOpen] of showPickerOpeners) {
    test(`file picker guard (${browserKind}): blocks ${deferral} showPicker activation`, async (page) => {
      ...
      const footprint = await page.evaluate(() => ({
        stableGlobal: Object.hasOwn(window, '__edithFilePickerGuardBridge'),
        attributes: Array.from(document.documentElement.attributes)
          .map(attribute => attribute.name)
          .filter(name => name.startsWith('data-edith-file-picker-')),
      }));
...
test('Chrome CDP file picker guard blocks trusted showPicker activation and restores the prototype', async (page) => {
  ...
  await page.evaluate(() => {
    ...
    root.setAttribute('data-edith-file-picker-guard', 'residual-content-guard');
    document.dispatchEvent(new Event('edith:file-picker-guard-arm'));
    root.removeAttribute('data-edith-file-picker-guard');
  });
...
test('set_checked (chrome): markers are one-shot, unique, and self-cleaning', async (page) => {
  ...
  const expiredCount = await page.evaluate((marker) => {
    ...
    return document.querySelectorAll(`[data-edith-set-checked-target="${marker}"]`).length;
  }, expiring.marker);
  ...
  await page.evaluate((marker) => {
    document.getElementById('trusted-firefox-checkbox')
      .setAttribute('data-edith-set-checked-target', marker);
  }, ambiguous.marker);
  ...
  const duplicateResolution = await uniqueClient.resolveSelector(
    42,
    `[data-edith-set-checked-target="${ambiguous.marker}"]`,
  ...
  const remaining = await page.locator(`[data-edith-set-checked-target="${ambiguous.marker}"]`).count();
  if (
