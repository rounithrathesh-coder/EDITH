...
what the EDITH extension actually sends.
...
└── _redact-trace.mjs           # convert a real edith-trace JSON → scenario stub (PII-scrubbed)
```
...
Use the redactor to bootstrap a scenario stub from a `edith-trace-*.json`
...
```
node test/llm/_redact-trace.mjs ~/Downloads/edith-trace-foo.json \
  --challenge-step 5 \
