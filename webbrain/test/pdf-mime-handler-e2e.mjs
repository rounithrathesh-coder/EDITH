...
function createMinimalPdf() {
  const pageContent = 'BT\n/F1 12 Tf\n20 100 Td\n(EDITH PDF MIME handler test) Tj\nET\n';
  const objects = [
...
async function inspectPdfRouting(context, url, extensionId, waitMs = 2000) {
  ...
  try {
    ...
    return {
      sawEDITHHandler: urls.some(value => value.startsWith(handlerUrl)),
...
async function assertHandlerTabSkipsContentTypeProbe(context, settings, extensionId, fixture) {
  ...
  try {
    ...
    // Exactly what the "Open PDF with EDITH" context menu does. That entry
...
async function main() {
  ...
  try {
    ...
    assert.ok(background, 'The EDITH service worker was not available for the PDF extraction test.');
    ...
    assert.match(extraction.result?.pages?.[0] || '', /EDITH PDF MIME handler test/);
    ...
    assert.equal(initialState.stored.pdfViewerEnabled, undefined, 'Fresh installs must preserve an unset EDITH PDF setting.');
    ...
    assert.equal(disabledRouting.sawEDITHHandler, false, `Disabled PDF handling still entered EDITH: ${disabledRouting.urls.join(', ')}`);
    ...
    assert.equal(enabledRouting.sawEDITHHandler, true, `Enabled PDF handling did not enter EDITH: ${enabledRouting.urls.join(', ')}`);
    ...
    assert.equal(disabledAgainRouting.sawEDITHHandler, false, `Turning PDF handling off still entered EDITH: ${disabledAgainRouting.urls.join(', ')}`);
    ...
    console.log(`  ✓ Chrome ${browser.version()} keeps native PDF routing aligned with the EDITH toggle`);
