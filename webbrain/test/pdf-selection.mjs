...
async function testHandlerUsesChromeStreamAndTextLayer() {
  ...
  assert.match(source, /__edithSelectionShortcutConfig/);
...
async function testPdfResponseStreamingStopsAtTheByteLimit() {
  ...
  await assert.rejects(
    ...
    /larger than the EDITH viewer limit/,
  );
...
async function testPdfSelectionCarriesItsTabScope() {
  ...
  try {
    ...
    await page.addScriptTag({ content: `
      ...
      window.__edithSelectionShortcutConfig = {
...
async function testPdfSelectionShortcutRunsInHandlerFrame() {
  ...
  try {
    ...
    await frame.waitForFunction(() => typeof window.__edithSelectionShortcut?.getState === 'function', null, { timeout: 5000 });
    ...
    await frame.waitForFunction(() => window.__edithSelectionShortcut.getState().shortcutVisible, null, { timeout: 5000 });
    await frame.evaluate(() => window.__edithSelectionShortcut.submitPreset('summarize'));
