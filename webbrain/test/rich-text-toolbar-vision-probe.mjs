...
//
// Trace mode is preferred because an exported EDITH trace can provide the
...
Recommended trace example:
  node test/rich-text-toolbar-vision-probe.mjs \\
    --trace ~/Downloads/edith-trace-gpt-5.6-luna-run_....json \\
...
Manual image example (rect is in image pixels unless --viewport is supplied):
  node test/rich-text-toolbar-vision-probe.mjs \\
    ...
    --value "EDITH is an open-source browser agent..." \\
    --model qwen-vl

Options:
  --trace <path>             Extract the case from an exported EDITH trace
