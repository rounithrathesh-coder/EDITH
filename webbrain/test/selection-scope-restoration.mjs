...
for (const [label, AgentClass, session] of [
  ...
  assert.equal(enriched.edithSelectionScopeRestored, true, `${label}: corrected turn was not marked for one-shot consumption`);
  ...
  assert.equal(nextTurn.edithSelectionScopeRestored, undefined, `${label}: later turn reused the one-shot marker`);
  ...
  for (const [pathIndex, streaming] of [false, true].entries()) {
    for (const [caseIndex, testCase] of [
      ...
      agent._enrichUserMessageWithCurrentPage = async () => ({
        ...
        edithSelectionScopeRestored: true,
      });
