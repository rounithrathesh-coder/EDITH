...
async function runExtensionClientSmoke(context, fixtureUrl) {
  ...
  try {
    ...
    const edith = installed.extensions.find(extension => extension.id === extensionId);
    assert.equal(edith?.enabled, true, 'Chrome loaded the EDITH extension in a disabled state.');
    assert.equal(path.resolve(edith.path), EXTENSION_PATH);
    ...
    console.log(
      `PASS: EDITH extension ${edith.version} enforced feature/mode gates and exercised `
...
async function main() {
  ...
  try {
    ...
    await withPhaseTimeout(
      ...
      'EDITH extension WebMCP smoke test',
    );
  } finally {
