...
description: >
  A consensus-ranked, cost-aware EDITH planner test of thirteen OpenRouter models from OpenAI, Anthropic, Google, xAI, Poolside, MiniMax, Z.ai, DeepSeek, Thinking Machines, Qwen, Moonshot, and Tencent.
excerpt: >
...
titleTag: >
  US-China open-weight and frontier model benchmark - EDITH Blog
ogTitle: >
...
This was the **full-tier**, non-frozen planner suite as it existed at commit `7182c21f`: 100 Chrome first-action cases per model, 41 available EDITH tools, native structured tool calls, and no saved request bodies.
...
“Non-frozen” also matters. Earlier EDITH posts used a May 2026 frozen baseline and often reported agreement with a saved Sonnet reference. This run uses one pinned code checkout rather than that historical frozen snapshot. It is appropriate for comparing these thirteen same-payload runs with each other, not for splicing their percentages into older frozen tables.
...
DeepSeek and HY3 share the important downside: **their tested routes are text-only**. They can plan over URLs, accessibility trees, extracted page text, and prior tool state, but cannot serve as EDITH's only model when a canvas app, chart, broken accessibility tree, or screenshot contains the decisive information. Gemini can. GLM's tested route cannot, but its separate Vision NVFP4 checkpoint makes that limitation less structural than it first appears.
...
Tags: #OpenWeights #OpenRouter #DeepSeekV4 #TencentHY3 #GLM52 #KimiK3 #MiniMaxM3 #ClaudeSonnet5 #GPT56 #Gemini36 #Grok45 #Poolside #LagunaXS #Inkling #Qwen36 #RTX5090 #ToolCalling #BrowserAgent #EDITH
