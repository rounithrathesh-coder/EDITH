---
title: >
  EDITH 31 brings offline WebGPU vision to text-only models
slug: edith-31-offline-webgpu-vision
sortOrder: -170
date: 2026-08-14
readTime: 5 min read
description: >
  EDITH 31.0.0 adds optional offline vision powered by WebGPU and Liquid AI's sub-1GB LFM2.5-VL-450M, giving strong text-only models a local visual layer.
excerpt: >
  EDITH 31.0.0 can run a lightweight vision model locally through WebGPU, then pass its visual observations to the original text-only planner. That makes inexpensive models such as DeepSeek V4 Flash and Laguna S 2.1 more capable without turning them into larger multimodal checkpoints.
titleTag: >
  EDITH 31 offline WebGPU vision for text-only models - EDITH Blog
ogTitle: >
  EDITH 31 brings offline WebGPU vision to text-only models
ogDescription: >
  Liquid AI's sub-1GB LFM2.5-VL-450M gives EDITH an optional local visual layer while the user's original text model remains the planner.
twitterTitle: >
  EDITH 31: offline WebGPU vision for text-only models
twitterDescription: >
...
2. **EDITH converts the useful visual state into context.** The planner gets the information it needs without requiring native image input.
3. **The user's original model decides what to do.** Its checkpoint, serving setup, and text-only interface can remain unchanged.

In other words, EDITH supplies the eyes while the model you chose remains the brain.

## What it looks like in EDITH 31
...
<figure>
  <img src="/assets/edith-31-lfm25-vl-local-fallback-settings.png" alt="EDITH 31.0.0 Multimodal Vision settings with the LFM2.5-VL local fallback enabled" width="1586" height="1384" loading="lazy">
  <figcaption>EDITH 31.0.0 with LFM2.5-VL local fallback enabled. Screenshots stay on the device, while the original active provider remains responsible for planning and tool calls.</figcaption>
</figure>
...
It is not supposed to replace a frontier planner, and that is exactly why it fits this architecture. EDITH needs the local model to extract useful visual evidence: what text is visible, which control looks selected, where a dialog is blocking the page, whether an error state appeared, or what a chart and canvas are showing. The larger text model can handle the longer-horizon planning, tool selection, and reasoning.

The ONNX package EDITH uses downloads about 770 MB from Hugging Face on first use, then caches locally. The download runs in the background, so you can switch tabs or close Settings while it continues—just keep Chrome open. Once cached, it provides vision without sending screenshots to a remote vision API. If the main planner is local too, the complete EDITH workflow can remain on the device.
...
EDITH 31 changes the model-selection equation. Instead of moving from a strong, inexpensive text model to a larger or more expensive multimodal variant, users can keep the original DeepSeek V4 Flash, Laguna S 2.1, or another text-only model and let EDITH provide the missing visual context locally.
...
This is also a more sustainable way to improve EDITH. A lightweight visual layer can serve many different planners, so every strong text-only release becomes a more complete browser-agent option without requiring a new multimodal graft for each model.

## Available now in EDITH 31.0.0

Offline WebGPU vision is optional in EDITH 31.0.0. Users with compatible browser and device support can enable the local model under **Settings → Multimodal → Vision**, while existing local and hosted vision configurations remain available for people who prefer them or need a different capability profile.

We will continue sharing compatibility notes and real EDITH vision results as more people use the local fallback.

Most of all, thank you to **Liquid AI** for making LFM2.5-VL-450M open source and openly available. A capable vision model below one gigabyte is what makes this split architecture practical, and releasing it under the LFM Open License gives projects like EDITH a foundation to build on.

Tags: #EDITH #WebGPU #OfflineAI #LiquidAI #LFM25VL #LocalAI #BrowserAgent
