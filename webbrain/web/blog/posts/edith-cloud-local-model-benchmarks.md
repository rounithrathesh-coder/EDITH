---
title: >
  EDITH Compass is live, and these are the local models we are benchmarking next
slug: edith-cloud-local-model-benchmarks
sortOrder: 1
date: 2026-06-19
readTime: 5 min read
description: >
  EDITH Compass is now live in the latest main branch, so you can try EDITH without running a local LLM or bringing your own API key. The hosted route is request-limited, but it is enough to see how the browser agent feels. Next up: benchmarks for VibeThinker-3B, Gemma 4 12B Coder, DiffusionGemma 26B, and Cohere North-Mini-Code 30B across practical local-VRAM bands.
excerpt: >
  EDITH Compass is live in the latest main branch, which means you can try EDITH without a local LLM or API access. It is request-limited, but useful for a first look. We are also lining up the next local-model benchmark set by hardware band: 4-12GB, 12-24GB, and 24-64GB VRAM.
titleTag: >
  EDITH Compass is live, plus the next local model benchmark list - EDITH Blog
ogTitle: >
  EDITH Compass is live, plus the next local model benchmark list
ogDescription: >
  Try EDITH from the latest main branch without a local LLM or API key, then follow our next benchmarks for practical local models by VRAM tier.
twitterTitle: >
  EDITH Compass is live
twitterDescription: >
...
html: true
lede: >
  Two updates. First: **EDITH Compass is now live** in the latest `main` branch, so you can start using EDITH without a local LLM and without bringing your own API access. It is limited to a small number of hosted requests, but that is enough to see the product loop, test the browser-agent flow, and decide whether you want to wire in your own local or API-backed model. Second: our next benchmark pass will focus on the best local models for the hardware people actually have, grouped by VRAM.
---

## EDITH Compass is live

Until now, trying EDITH seriously meant having one of two things ready: a local model server, or an API key for a hosted model provider. That is still the best setup for sustained use, but it is too much friction for a first look.

The new cloud path removes that first-run blocker. Download and install the latest `main` branch from the [EDITH GitHub repo](https://github.com/edith-one/edith), then run EDITH as usual. You can use the built-in EDITH Compass route for a limited number of requests without configuring a local LLM or external API provider.

This is not meant to be an unlimited hosted tier. The point is simpler: you should be able to install EDITH, try the browser agent, and see whether the workflow makes sense before spending time on model setup.

<div class="callout">
<strong>Recommended path:</strong> install from the latest <code>main</code> branch, try the limited EDITH Compass requests first, then switch to a local or API-backed model when you are ready for heavier usage.
</div>
...
This is probably the most important tier for EDITH users with a single consumer GPU. A good 12B coder can be large enough to reason about messy pages and small enough to run locally without turning every action into a wait.
...
0xSero's broader recommendation list also called out GLM-5.2-REAP for very large multi-GPU setups. We are intentionally leaving that out of this first EDITH local benchmark pass.

That is not a judgment on the model. It is a scope decision. Once a setup needs something like multiple workstation GPUs or clustered small systems, it stops answering the question most EDITH users are asking: what should I run locally on the hardware I already have?
...
The cloud path gives everyone a low-friction way to try EDITH today. The local benchmark pass should make the next decision clearer: when you are ready to move beyond the limited hosted requests, which model should you run on your own hardware?
