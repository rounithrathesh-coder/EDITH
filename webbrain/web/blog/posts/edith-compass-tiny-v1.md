---
title: >
  Introducing EDITH Compass Tiny: edge-sized Compact tool routing for EDITH
slug: edith-compass-tiny-v1
sortOrder: -281
date: 2026-09-05
readTime: 6 min read
description: >
  EDITH Compass Tiny v1 is a private preview of a 2.6B model built for EDITH's Compact tool loop. In our first routing suite it matches Qwen3.6-35B-A3B on tool-name conformity and beats the local LFM2.5 base model it was trained from by eight routing outcomes, at a size meant for edge deployment.
excerpt: >
  A 2.6B model does not replace a 35B one at general reasoning. But inside EDITH's narrow, production-shaped Compact routing loop, Compass Tiny v1 hits the same tool-name conformity as Qwen3.6-35B-A3B and beats its matched local base model by eight routing outcomes.
titleTag: >
  Introducing EDITH Compass Tiny v1 - EDITH Blog
ogTitle: >
  EDITH Compass Tiny v1: Compact browser-tool routing at the edge
ogDescription: >
  A first look at EDITH's 2.6B Compact model preview, its tool-routing benchmark, and what an open-weights release would take.
twitterTitle: >
  EDITH Compass Tiny v1: an edge-sized Compact model
twitterDescription: >
  Compass Tiny v1 matches a 35B Qwen model on EDITH Compact tool-name routing. More tests and v1.1/v2 work are coming.
keywords:
  - EDITH Compass Tiny
  - browser agent
  - tool calling
...
lede: >
  **EDITH Compass Tiny v1 is our first edge-sized model built for the Compact browser-tool loop.** We are not claiming that 2.6B parameters match a 35B model at general reasoning. What this first suite shows is narrower: a focused, grounded routing policy can make a small model useful inside a real browser agent. Compass matches Qwen3.6-35B-A3B at picking the expected tool name, and it beats the LFM2.5-2.6B base model it was trained from by eight routing outcomes. Qwen3.8-27B is still the quality leader wherever its cost and footprint fit.
---
...
The model is tuned for three Compact behaviors in EDITH:
...
| **EDITH Compass Tiny v1** | **2.6B** | **81 / 100** | 4 | **42** |
...
That is a bounded result and we would rather report it as one. Compass is not making a broad general-intelligence jump over its base. EDITH-specific training moved the next-action routing policy in the direction we wanted, without making the model any bigger.
...
Compass is not positioned as a replacement for that class. The claim is smaller: a 2.6B specialist can already handle a well-scoped EDITH Compact decision, so every low-risk browser step no longer has to go through a much larger planner.
...
The working roadmap has Compass v1.1 and eventually v2, with more data, more targeted evaluations, and clearer release artifacts. If that work lands, this private preview becomes an open-weights option for teams that want EDITH-native Compact behavior without routing every action through a large cloud model.
...
Tags: #EDITH #CompassTiny #BrowserAgent #ToolCalling #EdgeAI #CompactMode #LFM25 #OpenWeights
