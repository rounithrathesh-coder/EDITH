...
description: >
  How EDITH turned Liquid AI's 450M vision-language model into a browser specialist using a reviewed 16,646-example first dataset, a targeted 50,000-example second dataset, LoRA, and a held-out 100-case test.
excerpt: >
  The raw 450M model started near zero on EDITH's strict browser contract. Two data rounds moved it to 44/100 in the deployed WebGPU package—above a 3B LFM run and into the same band as much larger Qwen and Gemma models.
titleTag: >
  Fine-Tuning LFM2.5-VL-450M for Browser Tasks - EDITH Blog
ogTitle: >
  From 16K to 50K screenshots: building EDITH VL 2 450M
ogDescription: >
  A reproducible account of the data, LoRA training, validation, ONNX export, and browser benchmark behind EDITH's 450M WebGPU vision model.
twitterTitle: >
  The fine-tuning story behind EDITH VL 2 450M
twitterDescription: >
...
lede: >
  **The first benchmark result was bad enough to be useful.** Off-the-shelf LFM2.5-VL-450M could recover fragments of browser state, but it almost never satisfied EDITH's exact six-section screenshot contract. We kept the model small and changed the data instead. A first training round built around a 20K target produced 16,646 accepted examples. A second round expanded the corpus to exactly 50,000 screenshots and targeted the remaining failures. The deployed result now passes 44 of the same 100 held-out cases—and runs inside the browser through WebGPU.
---

## Why start with LFM2.5-VL-450M?

The obvious way to improve vision quality is to use a larger model. For EDITH's local fallback, that answer is incomplete. The model has to download into an extension, fit alongside the main browser workload, keep screenshots on-device, and run on consumer WebGPU implementations.
...
The [suite is public on GitHub](https://github.com/esokullu/edith/tree/main/test/vision). Training and validation images were checked for exact and perceptual overlap with those 100 cases. The benchmark was run only after checkpoint selection; it was not used as a training loss or early-stopping oracle.
...
Each screenshot was paired with EDITH's production system prompt and a six-section teacher response. The response mattered as much as the image: a model can recognize a login screen yet still fail EDITH if it invents labels, omits blockers, or ignores the required sections.
...
The [50K dataset is published on Hugging Face](https://huggingface.co/datasets/edith-one/edith-vl-2-450M-dataset), with row-level provenance, split metadata, teacher identifiers, and leakage checks documented in its dataset card.
...
| EDITH V1 | 16,646 | 30/100 | 70.06% | 0 |
| **EDITH V2** | **50,000** | **36/100** | **74.99%** | **0** |
...
| --- | ---: | ---: | ---: | ---: |
| EDITH V2, PyTorch | 36/100 | 74.99% | — | 0 |
| **EDITH V2, ONNX Q4/FP16** | **44/100** | **76.24%** | **87/100** | **0** |
...
The [merged model](https://huggingface.co/edith-one/edith-vl-2-450M) and [WebGPU-ready ONNX package](https://huggingface.co/edith-one/edith-vl-2-450M-onnx) are both available on Hugging Face.
...
The result against LFM2.5-VL-3B is more dramatic. The 3B MLX run scored 0/100 strict passes and a 7.76% mean because it repeatedly ignored the required numbered six-section contract. EDITH V2 followed that contract far more reliably and scored 44/100 despite having roughly one-seventh as many parameters.

That does **not** prove that 450M is a generally stronger vision model than LFM 3B. It proves something more useful for product engineering: under EDITH's exact browser-observation prompt and grader, the smaller specialist is much more effective than the larger off-the-shelf checkpoint.
...
## A much less expensive EDITH stack

EDITH can split planning and vision instead of paying a multimodal API for every turn:
...
2. Route the minority of calls that actually need pixels to EDITH VL 2 450M locally through WebGPU.
...
Tags: #EDITH #LFM25VL #LiquidAI #FineTuning #LoRA #WebGPU #ONNX #BrowserVision #LocalAI #TeacherDistillation
