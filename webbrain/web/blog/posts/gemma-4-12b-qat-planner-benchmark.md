---
title: >
  Gemma 4 12B QAT is fast enough for EDITH, but Qwen 3.5 9B still routes a little better
slug: gemma-4-12b-qat-planner-benchmark
sortOrder: -5
date: 2026-06-20
readTime: 6 min read
description: >
  We tested google/gemma-4-12B-it-qat-w4a16-ct through vLLM against EDITH's frozen first-tool-call browser-agent harness at concurrency 8, then ran the same vision probe used in earlier local-model posts.
excerpt: >
  Gemma 4 12B QAT lands as a very fast local EDITH planner: 92/100 parsed calls, 14% exact, 33% tool-name match, 0.43s median per request, and a working but imperfect vision probe.
titleTag: >
  Gemma 4 12B QAT EDITH planner benchmark - EDITH Blog
ogTitle: >
  Gemma 4 12B QAT is a fast local EDITH planner
ogDescription: >
...
twitterTitle: >
  Gemma 4 12B QAT EDITH planner benchmark
twitterDescription: >
...
keywords:
  ...
  - Qwen 3.5 9B
lede: >
  We ran **google/gemma-4-12B-it-qat-w4a16-ct** through EDITH's frozen first-tool-call planner harness on the currently running local vLLM server at port 8000. The result is a useful middle tier: much faster than the older Gemma 4 12B Coder Fable5 Composer 2.5 run and stronger on routing, but still a small step behind Qwen 3.5 9B on strict first-action quality.
---
...
That means 100 single-turn browser-agent prompts, the May 23 Claude Sonnet 4.6 EDITH system prompt and 41-tool schema, legacy text-call compatibility, no native OpenAI `tools` field, and up to eight active requests at a time.
...
The run was intentionally saturated. With `--concurrency 8`, the harness keeps up to eight chat-completion requests in flight, which is why the whole 100-case replay finished in 7.7 seconds and why GPU utilization looks higher than a normal EDITH browser loop.
