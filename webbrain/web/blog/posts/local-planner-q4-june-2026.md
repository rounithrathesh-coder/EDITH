---
title: >
  DiffusionGemma hits 0.35s median in the EDITH local planner bench
slug: local-planner-q4-june-2026
sortOrder: 0
date: 2026-06-19
readTime: 9 min read
description: >
  DiffusionGemma, Gemma 4 12B Coder, Cohere North-Mini-Code, and VibeThinker-3B in EDITH's frozen first-tool-call browser-agent harness.
excerpt: >
  Gemma 4 12B Coder, North Mini Code, and DiffusionGemma all completed the frozen legacy tool-call bench through different serving paths. DiffusionGemma was the speed surprise under vLLM and also handled the vision probe, but its diffusion-style generation still needs more EDITH-specific reliability work. VibeThinker confirmed its own model-card warning: it is not a browser-agent tool-calling model.
titleTag: >
  DiffusionGemma hits 0.35s median in the EDITH local planner bench - EDITH Blog
ogTitle: >
  DiffusionGemma hits 0.35s median in the EDITH local planner bench
ogDescription: >
  Gemma 4 12B Coder, North Mini Code, and DiffusionGemma completed EDITH's frozen local planner run; DiffusionGemma is fast but not yet reliable enough for EDITH, and VibeThinker is not a tool-calling agent model.
twitterTitle: >
  DiffusionGemma hits 0.35s median in the EDITH local planner bench
twitterDescription: >
  A practical local-serving pass over four new browser-agent planner candidates for EDITH.
keywords:
  - EDITH
  - local LLM
  - browser agent
...
- The May 23 Claude Sonnet 4.6 EDITH system prompt and 41-tool schema, frozen.
...
But the EDITH score above comes from vLLM, not from the GGUF CLI path. For local llama.cpp users, this is still not drop-in. For vLLM users with enough VRAM, it is suddenly one of the most interesting candidates in the batch.
...
So the fair interpretation is narrow: VibeThinker may still be interesting for reasoning or competitive-programming prompts, but these EDITH planner results should not be used as evidence for or against its intended use case.
