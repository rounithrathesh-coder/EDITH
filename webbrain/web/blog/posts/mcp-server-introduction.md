---
title: >
  EDITH MCP: Give Claude Code, OpenCode, and Codex access to your real browser session
slug: mcp-server-introduction
sortOrder: 90
date: 2026-08-08
readTime: 11 min read
description: >
  The EDITH MCP server lets any MCP-capable coding agent — Claude Code, OpenCode, Codex, Cursor — delegate browser tasks to your real, already-authenticated browser. No headless login dances, no cookie transfers. Just ask your agent to fill out a form, check a dashboard, or read a page behind SSO, and it runs in the browser you actually use.
excerpt: >
  The EDITH MCP server bridges Claude Code, OpenCode, and Codex to your real, already-signed-in browser. No headless login walls. No cookie transfers. Just ask your coding agent to interact with any page you're logged into.
titleTag: >
  EDITH MCP: Claude Code + OpenCode + Codex in your real browser — EDITH Blog
ogTitle: >
  EDITH MCP: Claude Code, OpenCode, Codex in your real browser
ogDescription: >
  The EDITH MCP server lets any MCP-capable agent delegate browser tasks to your real, authenticated Chromium session. No headless login walls.
twitterTitle: >
  EDITH MCP: Claude Code, OpenCode, Codex in your real browser
twitterDescription: >
...
html: true
lede: >
  We built the EDITH MCP server so that any MCP-capable coding agent — Claude Code, OpenCode, Codex, Cursor — can delegate browser tasks to the user's real, already-authenticated Chromium session. Instead of spinning up a headless browser that starts logged out of everything, the agent hands a goal to EDITH running in the browser profile the user already uses, and the extension carries it out in the active tab. This post explains how it works, how it differs from other browser MCP approaches, and how to wire it up to your agent of choice.
---

```
┌──────────┐  stdio  ┌─────────────────┐  ws://127.0.0.1:17374  ┌──────────────┐
│ Claude   │ ──────▶ │  edith-mcp   │ ────────────────────▶ │  Extension   │
...
| | **EDITH MCP** | **Headless browser MCP** |
|---|---|---|
...
The key architectural difference is **trust boundary placement**. Headless tools expose granular browser primitives (`click`, `type`, `navigate`, `screenshot`) through MCP — that's the whole surface area. EDITH deliberately does **not**. It exposes six **task-level** tools instead:

| Tool | Purpose |
|---|---|
| `edith_run` | Delegate a browser task. `mode='ask'` is read-only; `mode='act'` can click and type, gated by in-browser approval. |
| `edith_extract` | Return authenticated page data matching a caller-supplied JSON Schema. Always uses read-only Ask mode. |
| `edith_status` | Poll a run, or list every run. |
| `edith_respond` | Answer a run sitting at `needs_user_input`. |
| `edith_abort` | Stop a run. Actions already taken are not undone. |
| `edith_connection` | Report whether the extension is attached, and how to fix it if not. |

This is not a limitation — it's a safety decision. EDITH's capability × origin permission gate runs inside the extension's agent loop (`_executeToolBatch`), not inside the MCP tool execution path. A headless MCP tool calling `click` directly bypasses every approval prompt the product is built on. Delegating a goal instead keeps the trust boundary in the browser, where the human is.

## Setup: register the MCP server

The server is published on npm as `@edith/mcp-server`. Add it to your MCP client's configuration:

### Claude Code

```bash
claude mcp add --transport stdio edith -- npx -y @edith/mcp-server
```
...
```json
{
  "mcp": {
    "edith": {
      "type": "local",
      "command": ["npx", "-y", "@edith/mcp-server"]
    }
  }
}
```
...
```bash
codex mcp add edith -- npx -y @edith/mcp-server
```
...
```json
{
  "mcpServers": {
    "edith": {
      "command": "npx",
      "args": ["-y", "@edith/mcp-server"]
    }
  }
}
```

After adding the config, restart the MCP client. On startup, it spawns `npx -y @edith/mcp-server` as a child process. That process binds to `127.0.0.1:17374` and waits for the extension to connect.

## Connecting the browser extension

The MCP server alone is not enough — you need the EDITH extension installed and pointed at it:

1. Install the [EDITH extension](https://edith.one) in Chrome, Edge, Brave, Opera, or Vivaldi and open the browser.
2. In **EDITH → Settings → General → Advanced → MCP**, set the URL to `ws://127.0.0.1:17374/extension` and enable it.
3. Restart your MCP client (or just restart the MCP server process) and ask the client to call `edith_connection` to verify.

The extension holds exactly one outbound bridge socket. Pointing it here means it is *not* pointed at EDITH Cloud (port `17373`) or the LM Studio plugin (port `17375`). Switch it under **Settings → General → Advanced → MCP** when you need to change destinations.
...
Once configured, the MCP client sees the six EDITH tools in its tool list. You interact with them naturally:
...
If a run stops with status `needs_user_input`, the extension is asking a clarifying question. Answer it with `edith_respond`, passing the `run_id` and `clarify_id` from the snapshot.

For a machine-readable result, use `edith_extract` with a JSON Schema. It
...
|---|---|---|
| `EDITH_BRIDGE_PORT` | `17374` | Port the extension connects to. |
| `EDITH_BRIDGE_PATH` | `/extension` | Path segment; must match the URL in Settings. |
| `EDITH_COMMAND_TIMEOUT_MS` | `30000` | Per-command reply timeout. |
| `EDITH_RUN_TIMEOUT_MS` | `300000` | Default ceiling for `edith_run` polling. |
| `EDITH_POLL_INTERVAL_MS` | `1000` | Status poll interval. |

## When to use EDITH MCP

Choose EDITH MCP when:
...
The extension runs with `<all_urls>` host permissions. The full threat model is in [`docs/security-model.md`](https://github.com/edith-one/edith/blob/main/docs/security-model.md). Key points:
...
The bridge itself is loopback-only. Connections from HTTP(S) pages and non-extension browser origins are rejected before they can replace the extension socket. Accepted connections must present the extension's `hello` frame with `client: "edith-extension"`.
