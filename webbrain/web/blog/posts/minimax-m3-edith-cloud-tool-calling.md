---
title: >
  MiniMax M3 and EDITH Compass 1.0 enter the frozen EDITH planner benchmark
slug: minimax-m3-edith-cloud-tool-calling
sortOrder: -10
date: 2026-06-21
readTime: 6 min read
description: >
  We ran MiniMax M3 through OpenRouter and EDITH Compass 1.0 through api.edith.one against EDITH's frozen 100-case browser-agent first-tool benchmark, then updated the Sonnet 4.6 comparison table.
excerpt: >
  MiniMax M3 landed at 75% Sonnet first-tool alignment, below the older MiniMax M2.7 run. EDITH Compass 1.0 reached 73% alignment with a much lower run cost, but higher latency in this test path.
titleTag: >
  MiniMax M3 and EDITH Compass 1.0 EDITH planner benchmark - EDITH Blog
ogTitle: >
  MiniMax M3 and EDITH Compass 1.0 in EDITH's frozen planner bench
ogDescription: >
  Two new hosted OpenAI-compatible runs join the EDITH first-tool benchmark: MiniMax M3 via OpenRouter and EDITH Compass 1.0 via api.edith.one.
twitterTitle: >
  MiniMax M3 and EDITH Compass 1.0 planner benchmark
twitterDescription: >
  MiniMax M3: 75% Sonnet alignment. EDITH Compass 1.0: 73%. Same frozen 41-tool EDITH harness.
keywords:
  - EDITH
  - MiniMax M3
  - EDITH Compass
  ...
  - LLM benchmark
lede: >
  We added two hosted OpenAI-compatible planner runs to EDITH's frozen first-tool benchmark: **minimax/minimax-m3** through OpenRouter and **edith-cloud 1.0** through `https://api.edith.one/v1`. The surprise is that MiniMax M3 does not beat the older MiniMax M2.7 result in this harness. EDITH Compass lands close behind M3 on Sonnet alignment, costs much less for the replay, but was much slower on this endpoint path.
---
...
node test/llm/run-llamacpp.mjs \
  --base https://api.edith.one/v1 \
  --model "edith-cloud 1.0" \
  --tag 2026-06-21-edith-cloud-final \
  --concurrency 1 \
  --timeout 180000 \
  --no-save-request \
  --freeze test/llm/freeze/baseline-2026-05-23.json
...
The clean read: MiniMax M3 is a solid hosted planner, but it is not an upgrade over our saved MiniMax M2.7 result for this specific first-tool benchmark. EDITH Compass 1.0 is slightly weaker on Sonnet alignment, but it is cheaper in the reported usage fields and produced more parsed tool calls.

Latency needs one caveat. MiniMax M3 ran with concurrency 2, matching the older OpenRouter-style run. EDITH Compass had to run with concurrency 1; a concurrency-2 attempt produced transport failures from this environment. So compare the median per-case latency directly, but treat wall time as endpoint-path-specific.
...
That does not mean M3 is the weaker model in general. This is a narrow first-tool browser-agent routing harness, frozen to an older EDITH schema. It does mean we should not silently replace M2.7 with M3 in the planner table just because the model name is newer.

## EDITH Compass 1.0

EDITH Compass 1.0 came in just below MiniMax M3 on Sonnet alignment:

| Model | Parsed calls | Exact | Name | Sonnet all | Sonnet tooled | Median | Cost |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| MiniMax M3 | 85/100 | 17% | 32% | 75.0% | 73.9% | 3.1s | $1.06 |
| EDITH Compass 1.0 | 90/100 | 16% | 35% | 73.0% | 72.8% | 8.8s | $0.12 |

The profile is different. EDITH Compass produced more tool calls and a slightly better ideal tool-name score than M3, but it aligned less often with Sonnet's first-tool choices and had much higher latency in this run. It also reported a much lower cost for the 100-case replay.
...
| 6 | EDITH Compass 1.0 | 73.0% | 72.8% | 90% | 90% | 16% | 35% | 8.8s |
...
EDITH Compass 1.0 sits one row lower, but it is the more useful product story here. It is a good entrant with a free tier, and it gives people a working EDITH path when they do not have a local LLM ready. It should also keep improving as we tune the browser-side optimizations around the hosted route.

## What changes

For hosted planner routing, I would keep MiniMax M2.7 in the table ahead of MiniMax M3 until M3 wins a rerun with a newer prompt or a different tool format. M3 is good, but the frozen EDITH result does not justify a default-model change, especially when the newer model is about twice the size.

For EDITH Compass, the result is more product-facing than leaderboard-facing. It passed the old structured tool interface cleanly, completed all 100 cases without transport errors in the final run, and stayed close to the hosted MiniMax tier on Sonnet alignment. The latency needs endpoint work, but the quality/cost shape is usable, and the free tier makes it a practical default for anyone who wants to try EDITH before setting up a local model.

The next fair test is the current production prompt and tool schema, not the May 23 frozen one. The frozen run answers "how does it compare with our historical rows?" The live-schema run answers "what should EDITH route to today?"

Tags: #MiniMaxM3 #EDITHCloud #OpenRouter #ToolCalling #BrowserAgent #LLMBenchmark
