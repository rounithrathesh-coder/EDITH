...
description: >
  Meta's open 30B Muse Glimmer beats every budget Qwen model in EDITH's browser-vision benchmark, then lands above GPT-5.6 Luna on comparable first-action planner metrics—at the cost of much slower hosted inference.
excerpt: >
...
titleTag: >
  Muse Glimmer 30B vision and browser-agent benchmark - EDITH Blog
ogTitle: >
  Muse Glimmer: Meta's 30B model tested on browser vision and tool use
ogDescription: >
  300 EDITH cases across screenshots, first actions, and multi-turn recovery. Muse beats budget Qwen vision and lands above GPT-5.6 Luna on common planner metrics.
twitterTitle: >
...
That positioning is unusually aligned with EDITH's workload: multimodal understanding, structured tool use, multi-step reasoning, and failure recovery in one locally deployable model. The OpenRouter route exposes a 131,072-token context. At test time its lowest listed route price was **$0.30 per million input tokens and $1.10 per million output tokens**; provider-specific prices can be higher.
...
The vision run used EDITH's production screenshot contract: the same 100 browser screenshots, fixed six-section system prompt, temperature 0, 800-token maximum, deterministic weighted checks, and critical-fact gating used in our [budget Qwen comparison](/blog/qwen-budget-vision-openrouter).

The text run used the current EDITH Chrome planner payload at full tier: 100 first-action cases, native structured tools, Act temperature 0.15, and no reasoning-effort override. One request failed in transport on the original pass; its isolated retry returned a valid tool call. We substitute that retry in the measurements below.
...
Our [thirteen-model American-Chinese frontier benchmark](/blog/american-chinese-open-model-frontier-gap-benchmark) ranked models primarily by leave-one-out peer consensus. Muse was run later, after EDITH's prompt and tool schema had grown, so inserting it into that exact consensus ranking would pretend the payloads were identical. We do not do that.
...
The combined result makes Muse more interesting than either table alone. It is not the best text planner and not the fastest VLM. It is a 30B open model that can plausibly do both jobs well, recover stale state unusually reliably, and run on hardware an individual can own. That is a strong foundation. Now it needs a local EDITH run to show whether Meta's consumer-hardware story can turn a 16-second hosted screenshot response into an interactive agent loop.
...
Tags: #MuseGlimmer #MetaAI #OpenWeights #OpenRouter #Qwen3VL #GPT56 #Luna #VisionLanguageModel #ToolCalling #BrowserAgent #LocalAI #EDITH
