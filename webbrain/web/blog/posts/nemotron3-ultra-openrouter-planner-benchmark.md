---
title: >
  Nemotron 3 Ultra is huge, free, and not a EDITH planner win
slug: nemotron3-ultra-openrouter-planner-benchmark
sortOrder: -45
date: 2026-07-08
readTime: 6 min read
description: >
  We ran nvidia/nemotron-3-ultra-550b-a55b:free through EDITH's frozen 100-case browser-agent first-tool benchmark on OpenRouter. The 550B-total MoE model completed the run, but landed well below the best planner rows.
excerpt: >
  Nemotron 3 Ultra completed EDITH's frozen planner benchmark through OpenRouter's free endpoint, but 81 parsed calls, 65% Sonnet alignment, and a 40.6s p95 keep it out of the planner shortlist.
titleTag: >
  Nemotron 3 Ultra OpenRouter EDITH planner benchmark - EDITH Blog
ogTitle: >
  Nemotron 3 Ultra is not a EDITH planner win
ogDescription: >
  NVIDIA's 550B-total Nemotron 3 Ultra free endpoint completed EDITH's frozen planner run, but trails Agents-A1, Hy3, Qwen 3.7 Plus, and older Qwen 3.6 rows.
twitterTitle: >
  Nemotron 3 Ultra EDITH planner benchmark
twitterDescription: >
...
That is not the same as being a good browser first-tool planner. EDITH's harness is narrower and harsher: the model sees a browser state, user instruction, and a 41-tool browser-control schema, then we score the first tool call only.
...
The bad news: this is not a strong EDITH planner row. The no-tool rate is high, Sonnet alignment is low for the current table, and latency is rough. The median is tolerable for a free frontier-scale model, but a 40.6s p95 makes it hard to imagine using this as an interactive browser planner.

## Against the nearby hosted rows

The obvious comparison is not the full 550B parameter count. It is the hosted planner rows EDITH can actually call through an OpenAI-compatible API.
...
The Hy3 comparison is especially clean because both rows used OpenRouter free variants. Hy3 produced 95 parsed calls, 73% Sonnet alignment, and a 9.16s p95. Nemotron produced 81 parsed calls, 65% Sonnet alignment, and a 40.6s p95. For EDITH's planner job, Hy3 is the stronger free OpenRouter candidate by a wide margin.
...
The no-tool issue matters. In normal chat, prose can be acceptable for a vague instruction like "log in" or "buy it." But in EDITH's planner protocol, ambiguity should usually become a `clarify` tool. Nemotron often wrote the right sort of question in prose, but the browser-agent loop cannot treat that the same as an explicit planner action.
...
| 9 | EDITH Compass 1.0 | 90/100 | 16/100 | 35/100 | 73.0% | 72.8% | 8.8s |
