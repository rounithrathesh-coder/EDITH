...
readTime: 6 min read
description: >
  We tested nvidia/Qwen3.6-27B-NVFP4 against EDITH's frozen browser-agent first-tool benchmark. It is far faster than the older Qwen 3.6 27B row and enters the top 10, but loses three all-case Sonnet-alignment points on boundary cases.
excerpt: >
  Qwen 3.6 27B NVFP4 gives EDITH a big local latency win: 96/100 parsed calls, 1.8s median latency, and a top-10 Sonnet-reference result. The planner quality story is more mixed.
titleTag: >
  Qwen 3.6 27B NVFP4 EDITH planner benchmark - EDITH Blog
ogTitle: >
  Qwen 3.6 27B NVFP4 enters EDITH's planner top 10
ogDescription: >
...
twitterTitle: >
  Qwen 3.6 27B NVFP4 EDITH benchmark
twitterDescription: >
...
The old row is still the main comparison point because it is the same model family and size in the saved EDITH benchmark table.
...
| 7 | EDITH Compass 1.0 | 90/100 | 16/100 | 35/100 | 73.0% | 72.8% | 8.8s |
...
For EDITH, I would treat this as a serious practical candidate with one caveat: pair it with stronger boundary handling. If the outer agent can guard ambiguous, destructive, and no-browser-needed turns, the NVFP4 serving profile is very attractive. Without that guard, the old Qwen row remains the cleaner planner by the benchmark's all-case metric.

Tags: #Qwen36 #NVFP4 #NVIDIA #vLLM #ToolCalling #BrowserAgent #EDITH
