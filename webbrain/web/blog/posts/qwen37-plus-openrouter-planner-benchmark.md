...
readTime: 6 min read
description: >
  We ran qwen/qwen3.7-plus through EDITH's frozen 100-case browser-agent planner benchmark on OpenRouter. It ties MiniMax M3 on all-case Sonnet alignment, beats it on tool-call discipline, and lands just below the Gemma 4 31B QAT, Qwen 3.6 27B, and MiniMax M2.7 cluster.
excerpt: >
  Qwen 3.7 Plus reached 75/100 Sonnet alignment, 95/100 parsed native tool calls, and the best ideal tool-name score in the top hosted slice of EDITH's frozen planner table.
titleTag: >
  Qwen 3.7 Plus OpenRouter EDITH planner benchmark - EDITH Blog
ogTitle: >
  Qwen 3.7 Plus challenges MiniMax M3 in EDITH's planner benchmark
ogDescription: >
...
twitterTitle: >
  Qwen 3.7 Plus EDITH planner benchmark
twitterDescription: >
...
keywords:
  ...
  - tool calling
lede: >
  We ran **qwen/qwen3.7-plus** through EDITH's frozen 100-case browser-agent first-tool benchmark on OpenRouter. The headline is not that it beats every saved row. It does not. The interesting result is more specific: Qwen 3.7 Plus looks like a better direct OpenRouter competitor to MiniMax M3, while still sitting just below the strongest Gemma 4 31B QAT, Qwen 3.6 27B, and MiniMax M2.7 rows.
---
...
Qwen 3.7 Plus and MiniMax M3 tie on the headline all-case Sonnet metric at 75/100. After that, Qwen's row is stronger almost everywhere that matters for EDITH's first turn:
...
MiniMax M3 keeps a slightly better median latency in the saved run, but the difference is small enough that quality and cost dominate the decision. For EDITH's frozen planner harness, Qwen 3.7 Plus is the better MiniMax M3 competitor.
...
That is the profile I want from a browser planner: it reads pages, navigates correctly, and picks valid EDITH tools without format drama.
...
Against **MiniMax M3**, Qwen 3.7 Plus looks better for EDITH's planner job. It ties M3 on all-case Sonnet alignment and beats it on parsed calls, exact matches, ideal-name matches, Sonnet-tooled alignment, and reported run cost. If the decision is "which OpenRouter-hosted model should compete with MiniMax M3 for browser-agent routing?", Qwen 3.7 Plus is the stronger row in this frozen run.
...
So the practical take is: Qwen 3.7 Plus is not the new EDITH planner king, but it is a very serious OpenRouter candidate and a better MiniMax M3 rival than I expected. For users who want a hosted model with clean structured tools, it belongs in the shortlist.

Tags: #Qwen37 #OpenRouter #MiniMaxM3 #MiniMaxM27 #Gemma4 #Qwen36 #ToolCalling #BrowserAgent #EDITH
